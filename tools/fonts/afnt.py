#!/usr/bin/env python3
"""Import Amiga bitmap strikes into AFNT and emit trusted ROM C tables."""
...
def code_label(glyph) -> int | None:
    for label in glyph.get_labels():
        if label.__class__.__name__ == "Codepoint":
            raw = bytes(label)
            if len(raw) != 1:
                raise ValueError("only single-byte Amiga source fonts are supported")
            return raw[0]
    return None
...
def load_amiga(path: Path):
    try:
        import monobit
    except ImportError as error:
        raise SystemExit(
            "Amiga import needs monobit 0.53; run with "
...
def build_afnt(args: argparse.Namespace) -> bytes:
    fonts = [load_amiga(path) for path in args.strikes]
    expected_codes = set(range(args.first_code, args.last_code + 1))
    ...
    for path, font in zip(args.strikes, fonts, strict=True):
        ...
        source_codes = codes
        glyph_sets.append((encoded, font.get_default_glyph()))

    assert source_codes is not None
    unicode_to_id = []
    for glyph_id, source_code in enumerate(source_codes):
        scalar = ord(bytes((source_code,)).decode(args.encoding))
...
def emit_c(data: bytes, output: Path) -> None:
    cmap, strikes, glyphs, bitmap = unpack_afnt(data)
    lines = []
    from io import StringIO
    out = StringIO()
    out.write("/* Generated by tools/fonts/afnt.py; do not edit. */\n")
    emit_array(out, "astra_ui_cmap_codepoints", [entry[0] for entry in cmap], "uint32_t")
    emit_array(out, "astra_ui_cmap_glyphs", [entry[1] for entry in cmap], "uint16_t")
    out.write("static const AstraUiStrike astra_ui_strikes[] = {\n")
    ...
    out.write("};\n\nstatic const AstraUiGlyph astra_ui_glyphs[] = {\n")
    ...
    out.write("};\n\n")
    emit_array(out, "astra_ui_bitmap", list(bitmap), "uint8_t", 12)
...
def main() -> int:
    ...
    importer = commands.add_parser("import-amiga")
    ...
    importer.add_argument("--encoding", required=True)
    importer.add_argument("--first-code", type=int, required=True)
    importer.add_argument("--last-code", type=int, required=True)
    ...
    try:
        if args.command == "import-amiga":
            ...
            emit_c(args.input.read_bytes(), args.output)
