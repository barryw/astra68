#ifndef ASTRA_RENDER_BATCH_H
#define ASTRA_RENDER_BATCH_H

#include <astra/display_capture.h>
#include <astra/display.h>
#include <stdint.h>

/*
 * A render batch is copied unchanged into Astraea's graphics arena. Commands,
 * descriptors, glyph masks, and source pixels therefore use the hardware's
 * native big-endian records and arena-relative offsets. The MC68040 builds the
 * records; it never expands them into framebuffer pixels.
 */
#define ASTRA_RENDER_BATCH_MAGIC UINT32_C(0x41524254) /* ARBT */
#define ASTRA_RENDER_BATCH_VERSION_1_1 UINT32_C(0x00010001)
#define ASTRA_RENDER_BATCH_VERSION_1_2 UINT32_C(0x00010002)
#define ASTRA_RENDER_BATCH_VERSION_1_3 UINT32_C(0x00010003)
#define ASTRA_RENDER_BATCH_PRESENT_CURSOR (UINT32_C(1) << 0)
#define ASTRA_RENDER_BATCH_HEADER_BYTES 64u
#define ASTRA_RENDER_BATCH_ARENA_OFFSET UINT32_C(0x00800000)
#define ASTRA_RENDER_BATCH_WORKSPACE_LIMIT UINT32_C(0x01000000)
/* The final RGB888 capture device owns exactly one frame at the arena top. */
#define ASTRA_RENDER_BATCH_MEDIA_LIMIT \
    (UINT32_C(0x20000000) - ASTRA_DISPLAY_CAPTURE_FRAME_BYTES)
#define ASTRA_DISPLAY_CAPTURE_ARENA_OFFSET ASTRA_RENDER_BATCH_MEDIA_LIMIT
#define ASTRA_RENDER_BATCH_BUFFER_BYTES \
    (ASTRA_RENDER_BATCH_WORKSPACE_LIMIT - ASTRA_RENDER_BATCH_ARENA_OFFSET)
#define ASTRA_RENDER_BATCH_SUBMISSION_OFFSET UINT32_C(0x00801000)
#define ASTRA_RENDER_BATCH_COMPLETION_OFFSET UINT32_C(0x00811000)
#define ASTRA_RENDER_BATCH_RESOURCE_OFFSET UINT32_C(0x00819000)
#define ASTRA_RENDER_BATCH_DATA_OFFSET ASTRA_RENDER_BATCH_RESOURCE_OFFSET
#define ASTRA_RENDER_BATCH_SCANOUT_BYTES \
    (ASTRA_DISPLAY_WIDTH * ASTRA_DISPLAY_HEIGHT * 2u)
#define ASTRA_RENDER_BATCH_SCANOUT_STRIDE UINT32_C(0x00400000)
#define ASTRA_RENDER_BATCH_SCANOUT0_OFFSET UINT32_C(0x00000000)
#define ASTRA_RENDER_BATCH_SCANOUT1_OFFSET ASTRA_RENDER_BATCH_SCANOUT_STRIDE
#define ASTRA_RENDER_BATCH_MIN_BYTES \
    (ASTRA_RENDER_BATCH_RESOURCE_OFFSET - ASTRA_RENDER_BATCH_ARENA_OFFSET)
#define ASTRA_RENDER_BATCH_MAX_BYTES ASTRA_RENDER_BATCH_BUFFER_BYTES

typedef struct AstraRenderBatchHeader {
    uint32_t magic;
    uint32_t version;
    uint32_t total_bytes;
    uint32_t command_count;
    uint32_t submission_ring_offset;
    uint32_t completion_ring_offset;
    uint32_t resource_generation;
    uint32_t scanout_offset;
    uint32_t presentation_flags;
    uint32_t cursor_x;
    uint32_t cursor_y;
    uint32_t cursor_flags;
    uint32_t window_scene_offset;
    uint32_t window_scene_bytes;
    uint32_t reserved[2];
} AstraRenderBatchHeader;

_Static_assert(sizeof(AstraRenderBatchHeader) ==
                   ASTRA_RENDER_BATCH_HEADER_BYTES,
               "render batch header ABI changed");
_Static_assert(ASTRA_RENDER_BATCH_SCANOUT_BYTES <=
                   ASTRA_RENDER_BATCH_SCANOUT_STRIDE,
               "scanout stride cannot hold a native frame");
_Static_assert((ASTRA_DISPLAY_CAPTURE_ARENA_OFFSET & UINT32_C(0xff)) == 0u,
               "capture frame must be 256-byte aligned");

#endif
