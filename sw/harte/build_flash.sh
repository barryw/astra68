...
echo "    CPU=TG68K030_MMU2 CPU_CLK_DIV_BIT=$CPU_CLK_DIV_BIT SDRAM_ENABLE=$SDRAM_ENABLE HDMI_ENABLE=$HDMI_ENABLE UART_BAUD=$UART_BAUD TARGET_FREQ_MHZ=$TARGET_FREQ_MHZ PNR_SEED=$PNR_SEED"
...
( cd fpga/soc/oss_flow && \
  ...
  PNR_SEED="$PNR_SEED" \
  SOC_BUILD_ID="$EXPECT" \
  SD_BOOT_ENABLE=0 bash mkbit.sh ../../../sw/harte/rom_harness.hex harte >/dev/null )
...
printf '`0x%s` | %s | sha256:%s | %s | %s [%s]\n' \
  ...
  "CPU=TG68K030_MMU2 CPU_CLK_DIV_BIT=$CPU_CLK_DIV_BIT SDRAM_ENABLE=$SDRAM_ENABLE HDMI_ENABLE=$HDMI_ENABLE UART_BAUD=$UART_BAUD TARGET_FREQ_MHZ=$TARGET_FREQ_MHZ PNR_SEED=$PNR_SEED FLASH_MODE=$FLASH_MODE" \
