...
static uint32_t loaded_srp;
static uint32_t loaded_urp;
static uint32_t loaded_tc;
static bool transparent_translation_disabled;
static uint32_t flush_count;
static uint32_t non_global_flush_count;
...
{
    transparent_translation_disabled = true;
}
...
static uint32_t page_descriptor(uint32_t root_physical, uint32_t address)
{
    uint32_t *root = physical_words(root_physical);
    uint32_t root_descriptor = root[(address >> 25) & 0x7fu];
    uint32_t *pointer;
    uint32_t pointer_descriptor;
    uint32_t *page_table;

    if ((root_descriptor & 3u) != 2u)
        return 0u;
    pointer = physical_words(root_descriptor & 0xfffff000u);
    pointer_descriptor = pointer[(address >> 18) & 0x7fu];
    if ((pointer_descriptor & 3u) != 2u)
        return 0u;
    page_table = physical_words(pointer_descriptor & 0xfffff000u);
    return page_table[(address >> 12) & 0x3fu];
}

static uint32_t *page_table_words(uint32_t root_physical, uint32_t address)
{
    uint32_t *root = physical_words(root_physical);
    uint32_t *pointer = physical_words(
        root[(address >> 25) & 0x7fu] & 0xfffff000u);

    return physical_words(
        pointer[(address >> 18) & 0x7fu] & 0xfffff000u);
}
...
static void initialize_test(void)
{
    ...
    transparent_translation_disabled = false;
    flush_count = 0u;
    non_global_flush_count = 0u;
    flush_page_count = 0u;
...
static void test_private_reservation_fault_and_decommit(void)
{
    ...
    assert(committed.free_frames + 3u == reserved.free_frames);
...
static void test_kernel_root_and_enable_sequence(void)
{
    ...
    assert(root[0] == 0u);
    assert((root[1] & 0x63u) == 0x62u);
    assert(page_descriptor(stats.kernel_root_physical, 0x02000000u) ==
           0x02000481u);
    ...
    assert(page_descriptor(stats.kernel_root_physical, 0x03c00000u) ==
           0x03c00481u);
    assert(page_descriptor(stats.kernel_root_physical, OHCI_DMA_POOL_BASE) ==
           (OHCI_DMA_POOL_BASE | 0x4e1u));
    assert(page_descriptor(stats.kernel_root_physical, 0x03fff000u) ==
           0x03fff4e1u);
    ...
    assert(page_descriptor(stats.kernel_root_physical, 0xfff00000u) ==
           0xfff004e1u);
    assert(page_descriptor(stats.kernel_root_physical, 0xfff20000u) ==
           0xfff204e1u);
    assert(page_descriptor(stats.kernel_root_physical, 0xfff40000u) ==
           0xfff404e1u);
    assert(page_descriptor(stats.kernel_root_physical, 0xfff30000u) == 0u);
    ...
    assert(loaded_srp == stats.kernel_root_physical);
    assert(loaded_urp == stats.empty_root_physical);
    assert(transparent_translation_disabled);
    assert(loaded_tc == KERNEL_PMMU_TC_4K);
    assert(function_code_sets == 1u && flush_count == 1u);
    assert(cache_invalidation_count == 1u);
    assert(kernel_vm_control_state(&control));
    assert(control.srp_table_address == loaded_srp);
    assert(control.urp_table_address == loaded_urp);
    ...
    const KernelVmStats *baseline_vm, uint32_t root_descriptor)
{
    ...
    assert(root[KERNEL_VM_AREA_BASE >> 25] == root_descriptor);
...
static void test_shared_map_transaction_rolls_back_every_stage(void)
{
    ...
    for (uint32_t index = 0u;
        ...
        root_descriptor = physical_words(space.root_physical)
            [KERNEL_VM_AREA_BASE >> 25];
...
static void test_shared_map_existing_leaf_rollback_and_alias_guards(void)
{
    ...
    assert(kernel_memory_alloc_pages_zeroed(
               2u, KERNEL_FRAME_SHARED, frame_owner, physical_pages) ==
           KERNEL_MEMORY_OK);
    root_descriptor = physical_words(first.root_physical)
        [KERNEL_VM_AREA_BASE >> 25];
    assert((root_descriptor & 3u) == 2u);
    table = page_table_words(first.root_physical, KERNEL_VM_AREA_BASE);
     ...
     * on. The native 68040 walk also lets the range cross hardware page-table
     * boundaries without exposing that geometry to callers.
     */
    assert(kernel_vm_map_shared_range(
    ...
    /*
     * One page below a native 68040 256 KiB page-table boundary, two pages
     * long. The range stays in one area slot while crossing hardware tables.
     */
    assert(kernel_vm_map_shared_range(
               &first, KERNEL_VM_AREA_BASE + 0x00040000u - KERNEL_PAGE_SIZE,
               physical_pages, 2u, frame_owner, KERNEL_VM_READ) ==
           KERNEL_VM_OK);
    assert(kernel_vm_unmap_shared_range(
               &first, KERNEL_VM_AREA_BASE + 0x00040000u - KERNEL_PAGE_SIZE,
               physical_pages, 2u, frame_owner) == KERNEL_VM_OK);
...
static void test_device_aperture_above_the_low_region_is_uncached(void)
{
    ...
    assert(kernel_vm_init() == KERNEL_VM_OK);
    assert(kernel_vm_stats(&stats));
    assert(page_descriptor(stats.kernel_root_physical, 0x05f00000u) ==
           0x05f004e1u);
    assert(page_descriptor(stats.kernel_root_physical, 0x05e00000u) ==
           0x05e00481u);
    assert(page_descriptor(stats.kernel_root_physical, 0x04000000u) ==
           0x04000481u);
}
