...
static void test_all_motorola_formats(void)
{
    ...
    for (uint32_t index = 0u; index < sizeof(formats); ++index) {
        ...
        write_be16(raw, 0u, index == 0u ? 0x2000u : 0x001fu);
        write_be32(raw, 2u, 0x12345678u + index);
        write_be16(raw, 6u,
                   (uint16_t)((uint16_t)formats[index] << 12) | 0x0080u);
        if (formats[index] == 0x2u)
            write_be32(raw, 8u, 0x87654321u);
        if (formats[index] == 0xau || formats[index] == 0xbu) {
            write_be16(raw, 10u, 0x0151u);
            write_be32(raw, 16u, 0xdffffff8u);
            write_be32(raw, 24u, 0xc0dec0deu);
        }
        if (formats[index] == 0xbu)
            write_be32(raw, 36u, 0x000001a6u);
...
static void test_access_fault_fixup_changes_only_pc(void)
{
    ...
    write_be32(raw, 2u, 0x02012340u);
    write_be16(raw, 6u, 0xb008u);
    write_be16(raw, 10u, 0x0141u);
    write_be32(raw, 16u, 0x10001fffu);
