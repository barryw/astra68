...
_Static_assert(KERNEL_VM_DMA_SLOT_COUNT *
                   (KERNEL_VM_DMA_SLOT_SIZE / KERNEL_PAGE_SIZE) <= UINT16_MAX,
               "process DMA page accounting is too narrow");
...
typedef struct KernelHostChannel {
...
typedef struct KernelProcess {
    ...
    uint32_t qualification_authorized_sources;
    uint32_t qualification_completed_sources;
     ...
     * KERNEL_PROCESS_THREAD_SLOT_COUNT; sixty-four costs six bytes a process.
     */
    ...
    uint8_t qualification_authorized;
...
static uint32_t exiting_process_count;
 ...
 */
_Static_assert(
    KERNEL_PROCESS_THREAD_SLOT_COUNT * KERNEL_THREAD_STACK_PAGES_MAX <=
        UINT16_MAX,
...
_Static_assert(KERNEL_PROCESS_THREAD_SLOT_COUNT <= 64u,
...
_Static_assert(ASTRA_RIGHT_READ == KERNEL_PROCESS_RIGHT_QUERY &&
                   ...
                   ASTRA_RIGHT_SIGNAL == KERNEL_PROCESS_RIGHT_SIGNAL &&
                   ASTRA_RIGHT_WAIT == KERNEL_PROCESS_RIGHT_WAIT &&
                   ASTRA_RIGHT_TRANSFER == KERNEL_PROCESS_RIGHT_TRANSFER &&
...
typedef struct KernelProcessSoakState {
    ...
    uint32_t baseline_metadata_frames;
...
static bool soak_metadata_frames(uint32_t *frames)
{
    ...
    if (frames == NULL)
        return false;
    for (uint32_t site = KERNEL_ALLOCATION_SITE_DMA_METADATA;
         site < KERNEL_ALLOCATION_SITE_COUNT; ++site) {
        if (!kernel_allocation_site_stats((KernelAllocationSite)site,
                                          &stats) ||
            !astra_u32_add_checked(total, stats.current_units, &total))
            return false;
    }
    *frames = total;
    return true;
    ...
    while (physical != 0u) {
        ...
        for (uint32_t index = 0u; index < library->plan.segment_count;
            ...
            const KernelElfSegment *segment =
                kernel_elf_image_segment(&library->plan, index);
            uint32_t base;
...
static bool process_handle_retain(void *object, void *context)
{
    (void)context;
    return retain_process_handle(object) == KERNEL_PROCESS_OK;
}
...
static bool process_pool_valid(void)
{
    ...
    for (uint32_t slot = 0u; slot < process_slots; ++slot) {
        ...
        if (references != process->handle_references)
            return false;
        if (process->suspended > 1u ||
            (process->suspended != 0u &&
             process->process_state != KERNEL_PROCESS_CREATED &&
             process->process_state != KERNEL_PROCESS_RUNNING))
            return false;
    ...
    return observed_interval_next == interval_next_deadline &&
           observed_exiting == exiting_process_count;
}
                                                ...
                                                uint16_t *created_slot)
{
    ...
    *created_slot = PROCESS_SLOT_NONE;
    ...
    for (uint32_t attempt = 0u; attempt < ASTRA_PROCESS_ID_MAX; ++attempt) {
        bool used = false;
        uint32_t candidate = next_process_id;

        if (candidate == 0u || candidate > ASTRA_PROCESS_ID_MAX)
            candidate = 1u;
...
static void process_name_set(KernelProcess *process,
                             const AstraLaunchArguments *arguments,
                             const char *argument_bytes)
{
    uint32_t length = 0u;
    ...
    while (length + 1u < sizeof(process->name) &&
           length < arguments->length && argument_bytes[length] != '\0') {
        process->name[length] = argument_bytes[length];
        ++length;
    }
    process->name[length] = '\0';
}

static uint64_t interval_timer_earliest(void)
{
    ...
    for (uint32_t index = 0u; index < process_slots; ++index) {
        ...
        if (process != NULL &&
            ...
            earliest = process->interval_deadline;
    }
    return earliest;
}
...
/*
 * Queue one notification and interrupt only the process's selected receiving
 * thread. Pending standard notifications coalesce in the bitset. A blocked
 * notification stays pending without producing a spurious EINTR.
 */
static KernelProcessStatus queue_process_signal(KernelProcess *process,
                                                uint32_t signal,
                                                bool *woke)
{
    ...
    if (woke != NULL)
        *woke = false;
    if (process == NULL || signal == 0u || signal >= 32u ||
        (process->process_state != KERNEL_PROCESS_CREATED &&
         process->process_state != KERNEL_PROCESS_RUNNING))
        return KERNEL_PROCESS_INVALID_ARGUMENT;
    ...
    if (target != NULL && target->state == KERNEL_THREAD_BLOCKED) {
        if (kernel_thread_cancel_wait(target, ASTRA_SYSCALL_CANCELLED) !=
            KERNEL_THREAD_OK)
            return KERNEL_PROCESS_CORRUPT;
        if (woke != NULL)
            *woke = true;
...
static void scheduler_timer_rearm_at(uint64_t now)
{
    ...
    if (kernel_sync_next_timer_deadline(&deadline) && deadline < target)
        target = deadline;
    if (interval_next_deadline < target)
        target = interval_next_deadline;
    ...
    uint64_t now, uint32_t *expired_threads, uint8_t *highest_priority)
{
    ...
    if (interval_next_deadline <= now) {
        for (uint32_t index = 0u; index < process_slots; ++index) {
            ...
            if (queue_process_signal(process, ASTRA_SIGNAL_ALARM, &woke) !=
                KERNEL_PROCESS_OK)
                return KERNEL_PROCESS_CORRUPT;
            if (woke) {
                ++expired;
                if (!kernel_thread_highest_ready_priority(&highest))
                    return KERNEL_PROCESS_CORRUPT;
            }
        }
                                           ...
                                           KernelThread **thread)
{
    ...
    if (kernel_thread_note_kernel_entry(current_thread,
        ...
        KERNEL_THREAD_OK)
        return KERNEL_PROCESS_CORRUPT;
    if ((format0_vector == UINT16_MAX ?
             kernel_context_capture(&current_thread->context, registers,
                                    user_stack, raw_frame) :
             kernel_context_capture_format0(
                 &current_thread->context, registers, user_stack, raw_frame,
                 format0_vector)) != KERNEL_CONTEXT_OK)
...
static bool ready_thread_preempts(const KernelThread *thread,
                                  bool allow_equal_priority)
{
    uint8_t priority;
    ...
    return thread != NULL &&
           ...
           (priority > thread->effective_priority ||
            (allow_equal_priority &&
             priority == thread->effective_priority));
}

static bool ready_thread_outranks(const KernelThread *thread)
{
    return ready_thread_preempts(thread, false);
}
                                            ...
                                            bool allow_equal_priority)
{
    ...
    if (quantum_preempt_pending == 0u &&
        deadline_preempt_pending == 0u &&
        !ready_thread_preempts(previous, allow_equal_priority)) {
...
static KernelProcessStatus finish_reap(KernelProcess *process)
{
    ...
    if (process->handles_closed == 0u) {
        ...
        (void)host_channels_close_process(process);
        if (process->host_used != 0u) {
            kernel_platform_host_release_owner(process->owner);
            process->host_used = 0u;
        }
    ...
    if (exiting_process_count == 0u)
        return KERNEL_PROCESS_CORRUPT;
    --exiting_process_count;
    process->process_state = KERNEL_PROCESS_DEAD;
...
static KernelProcessStatus finish_thread_reaps(void)
{
    for (uint32_t slot = 0u; slot < kernel_thread_slot_limit(); ++slot) {
        ...
        if (process->process_state != KERNEL_PROCESS_CREATED &&
            process->process_state != KERNEL_PROCESS_RUNNING)
            return KERNEL_PROCESS_CORRUPT;
        if (thread->stack_slot >= KERNEL_PROCESS_THREAD_SLOT_COUNT)
            return KERNEL_PROCESS_CORRUPT;
        performance = kernel_performance_begin(
            KERNEL_PERFORMANCE_THREAD_REAP);
        if (host_channel_close(process, thread) != ASTRA_SYSCALL_OK) {
            kernel_performance_end(performance);
            return KERNEL_PROCESS_CORRUPT;
        }
        ...
        if (thread->stack_released == 0u) {
            ...
            for (uint32_t page = 0u; page < thread->tls_pages; ++page) {
                if (kernel_vm_unmap_page(
                        &process->address_space,
                        thread->tls_base + page * KERNEL_PAGE_SIZE) !=
                    KERNEL_VM_OK) {
                    kernel_performance_end(performance);
                    return KERNEL_PROCESS_CORRUPT;
                }
...
static uint32_t block_lease_info(AstraBlockLeaseInfo *geometry)
{
    ...
    geometry->queue_depth = state.queue_depth;
                              ...
                              uint32_t syscall)
{
...

    {
         ...
         */
        if (kernel_block_service(&serviced) != KERNEL_BLOCK_OK)
            return ASTRA_SYSCALL_IO_ERROR;
        thread->context.data[1] = serviced;
...
typedef uint32_t (*HostTransportExecute)(uint32_t owner,
                                         uint32_t physical_buffer,
                                         uint32_t byte_size,
                                         uint32_t command_count,
                                         uint32_t *executed_commands);

static uint32_t execute_host_transport(
    KernelProcess *process, uint32_t buffer_handle, uint32_t buffer_offset,
    uint32_t byte_size, uint32_t command_count, uint32_t reserved,
    uint32_t command_size, uint32_t maximum_transfer,
    uint32_t maximum_commands, uint32_t device_generation,
    HostTransportExecute execute, uint32_t *executed_out)
{
    KernelProcessDmaBuffer *buffer = NULL;
    KernelHandleStatus handle_status;
    KernelDmaToken token;
    ...
    if (process == NULL || execute == NULL || executed_out == NULL ||
        command_count == 0u || reserved != 0u || byte_size == 0u ||
        command_size == 0u || command_count > byte_size / command_size ||
        command_count > maximum_commands || byte_size > maximum_transfer ||
        (buffer_offset & (ASTRA_ABI_ALIGNMENT - 1u)) != 0u)
        return ASTRA_SYSCALL_INVALID_ARGUMENT;
    handle_status = kernel_handle_lookup(
        process->handles, buffer_handle, KERNEL_OBJECT_DMA,
        ASTRA_RIGHT_READ | ASTRA_RIGHT_WRITE, (void **)&buffer);
    ...
    if (buffer_offset > buffer_bytes ||
        byte_size > buffer_bytes - buffer_offset)
        return ASTRA_SYSCALL_INVALID_ARGUMENT;
    if (kernel_dma_begin(buffer->dma, process->owner, buffer_offset, byte_size,
                         KERNEL_DMA_BIDIRECTIONAL, device_generation,
                         &token) != KERNEL_DMA_OK)
        return ASTRA_SYSCALL_WOULD_BLOCK;
    status = execute(process->owner, token.physical_address, byte_size,
                     command_count, &executed);
    if (kernel_dma_complete(&token) != KERNEL_DMA_OK)
        return ASTRA_SYSCALL_IO_ERROR;
    if (status != ASTRA_SYSCALL_OK)
        return status;
    if (executed != command_count)
        return ASTRA_SYSCALL_IO_ERROR;
    *executed_out = executed;
    return ASTRA_SYSCALL_OK;
}

static uint32_t host_channel_close_slot(KernelProcess *process,
                                        uint32_t slot)
{
    ...
    if (channel->active == 0u)
        return ASTRA_SYSCALL_OK;
    if (channel->process_id != process->id)
        return ASTRA_SYSCALL_ACCESS_DENIED;
    if (channel->waiting != 0u) {
        uint32_t woken;

        if (kernel_thread_wake_all(&channel->waiters,
                                   ASTRA_SYSCALL_PEER_DEAD, &woken) !=
            KERNEL_THREAD_OK || woken > 1u)
            result = ASTRA_SYSCALL_IO_ERROR;
        channel->waiting = 0u;
    }
    if (kernel_platform_host_channel_close(
            process->owner, channel->host_generation,
            channel->channel_generation, slot) != ASTRA_SYSCALL_OK)
        result = ASTRA_SYSCALL_PEER_DEAD;
    if (kernel_vm_unmap_page(&process->address_space,
                             channel->virtual_address) != KERNEL_VM_OK)
        result = ASTRA_SYSCALL_IO_ERROR;
    if (kernel_dma_abort(&channel->token) != KERNEL_DMA_OK)
        result = ASTRA_SYSCALL_IO_ERROR;
    kernel_bytes_clear(channel, sizeof(*channel));
    kernel_thread_wait_queue_init(&channel->waiters);
    return result;
}
...
static uint32_t host_channels_close_process(KernelProcess *process)
{
    ...
    for (uint32_t slot = 0u; slot < KERNEL_VM_HOST_CHANNEL_PAGE_COUNT;
        ...
        if (host_channels[slot].active == 0u ||
            host_channels[slot].process_id != process->id)
            continue;
        if (host_channel_close_slot(process, slot) != ASTRA_SYSCALL_OK)
            result = ASTRA_SYSCALL_IO_ERROR;
    }
    return result;
}
    ...
    uint32_t user_address)
{
    ...
    KernelDmaToken token;
    uint32_t slot;
    uint32_t physical_doorbell;
    uint32_t command_bytes;
    int copy_status;

    if (process == NULL || channel == NULL || state == NULL ||
        thread == NULL ||
        thread->stack_slot >= KERNEL_PROCESS_THREAD_SLOT_COUNT ||
        thread->process_id != process->id ||
        ...
        channel->size != sizeof(*channel) || channel->flags != 0u ||
        channel->channel_generation != 0u ||
        channel->channel_address != 0u || channel->host_generation != 0u ||
        channel->byte_size > state->maximum_transfer ||
        channel->command_capacity == 0u ||
        (channel->command_capacity & (channel->command_capacity - 1u)) != 0u ||
        channel->command_capacity > state->maximum_commands)
        return ASTRA_SYSCALL_INVALID_ARGUMENT;
    for (uint32_t index = 0u;
         index < sizeof(channel->reserved) / sizeof(channel->reserved[0]);
         ++index) {
        if (channel->reserved[index] != 0u)
            return ASTRA_SYSCALL_INVALID_ARGUMENT;
    }
    if (channel->command_capacity >
    ...
    if (handle_status != KERNEL_HANDLE_OK || buffer == NULL ||
        buffer->active == 0u)
        return ASTRA_SYSCALL_IO_ERROR;
    if (channel->byte_size >
        (uint32_t)buffer->page_count * KERNEL_PAGE_SIZE)
        return ASTRA_SYSCALL_INVALID_ARGUMENT;
    if (kernel_dma_begin(buffer->dma, process->owner, 0u,
                         channel->byte_size, KERNEL_DMA_BIDIRECTIONAL,
    ...
    channel->channel_address = KERNEL_VM_HOST_CHANNEL_BASE +
                               thread->stack_slot * KERNEL_PAGE_SIZE;
    if (kernel_vm_map_host_channel_page(&process->address_space,
                                        channel->channel_address,
                                        physical_doorbell) != KERNEL_VM_OK) {
        (void)kernel_platform_host_channel_close(
            process->owner, state->host_generation, thread->generation,
            slot);
        (void)kernel_dma_abort(&token);
    ...
    host_channels[slot].command_capacity = channel->command_capacity;
    kernel_thread_wait_queue_init(&host_channels[slot].waiters);
    host_channels[slot].active = 1u;
    process->host_used = 1u;
    channel->channel_generation = thread->generation;
    channel->host_generation = state->host_generation;
    copy_status = kernel_copy_to_user(user_address, channel,
                                      sizeof(*channel));
    if (copy_status != KERNEL_USER_COPY_OK) {
        (void)host_channel_close_slot(process, slot);
        return ASTRA_SYSCALL_BAD_ADDRESS;
    }
    return ASTRA_SYSCALL_OK;
}

static bool host_channel_result(const KernelHostChannel *channel,
                                uint32_t expected_consumer,
                                uint32_t *result)
{
    KernelPlatformHostChannelState state;

    if (!kernel_platform_host_channel_completion(
            channel->token.physical_address, channel->token.byte_count,
            channel->command_capacity, &state) ||
        (state.state_flags & ASTRA_HOST_STATE_READY) == 0u ||
        state.generation != channel->channel_generation) {
        *result = ASTRA_SYSCALL_PEER_DEAD;
        return true;
    }
    ...
    if ((int32_t)(state.consumer_position - expected_consumer) >= 0) {
...
static KernelProcessStatus host_channel_wait(
    KernelProcess *process, KernelThread *thread, uint32_t consumer_position,
    uint64_t now, uint64_t deadline, bool *blocked, uint32_t *result)
{
    KernelHostChannel *channel;
    ...
    *blocked = false;
    ...
    /* A running owner cannot still be asleep on its previous synchronous op. */
    channel->waiting = 0u;
    ...
    wait_status = kernel_thread_block_until(
        thread, &channel->waiters, sequence, now, deadline,
                                ...
                                uint32_t device_generation)
{
    ...
    }
    {
        ...
        if (request.size != sizeof(request))
            return ASTRA_SYSCALL_INVALID_ARGUMENT;
        status = execute_host_transport(
            process, request.buffer, request.buffer_offset,
            request.byte_size, request.command_count, request.reserved,
            ASTRA_NETWORK_HOST_COMMAND_SIZE, state.maximum_transfer,
            state.maximum_transfer / ASTRA_NETWORK_HOST_COMMAND_SIZE,
            device_generation, kernel_platform_network_execute, &executed);
        if (status != ASTRA_SYSCALL_OK)
            return status;
...
static uint32_t host_syscall(KernelProcess *process, KernelThread *thread,
                             uint32_t syscall, uint32_t device_generation)
{
    uint32_t user_address = thread->context.data[2];
    KernelPlatformHostState state;
    int copy_status;

    if (syscall == ASTRA_SYSCALL_HOST_CHANNEL_CLOSE)
        return host_channel_close(process, thread);
    if ((user_address & (ASTRA_ABI_ALIGNMENT - 1u)) != 0u)
        return ASTRA_SYSCALL_INVALID_ARGUMENT;
    if (!kernel_platform_host_state(&state) ||
        (state.state_flags & ASTRA_HOST_STATE_READY) == 0u)
        return ASTRA_SYSCALL_PEER_DEAD;
    if (syscall == ASTRA_SYSCALL_HOST_QUERY) {
        ...
        info.host_generation = state.host_generation;
        info.maximum_transfer = state.maximum_transfer;
        info.maximum_commands = state.maximum_commands;
        copy_status = kernel_copy_to_user(user_address, &info, sizeof(info));
        return copy_status == KERNEL_USER_COPY_OK ? ASTRA_SYSCALL_OK :
               copy_status == KERNEL_USER_COPY_BAD_ADDRESS ||
                       copy_status == KERNEL_USER_COPY_INVALID_ARGUMENT ?
    ...
    if (syscall == ASTRA_SYSCALL_HOST_CHANNEL_OPEN) {
        AstraHostChannelOpen channel;

        copy_status = kernel_copy_from_user(&channel, user_address,
                                            sizeof(channel));
        if (copy_status != KERNEL_USER_COPY_OK)
            return ASTRA_SYSCALL_BAD_ADDRESS;
        return host_channel_open(process, thread, &channel, device_generation,
                                 &state, user_address);
    }
    {
        ...
        status = execute_host_transport(
            process, request.buffer, request.buffer_offset,
            request.byte_size, request.command_count, request.reserved,
            ASTRA_HOST_COMMAND_SIZE, state.maximum_transfer,
            state.maximum_commands, device_generation,
            kernel_platform_host_execute, &executed);
        if (status != ASTRA_SYSCALL_OK)
            return status;
        thread->context.data[1] = executed;
        return ASTRA_SYSCALL_OK;
...
static KernelProcessStatus retire_process(KernelProcess *retiring,
                                          KernelProcessExitReason reason,
                                          uint32_t exit_status,
                                          KernelCpuContext **next_context)
{
    KernelProcessStatus status;
    ...
    else if (reason == KERNEL_PROCESS_EXIT_SIGNAL && exit_status != 0u &&
             exit_status < 32u)
        retiring->exit_status = ASTRA_STATUS_SIGNALLED(exit_status);
...
{
    return KERNEL_PROCESS_RIGHT_QUERY | KERNEL_PROCESS_RIGHT_TERMINATE |
           KERNEL_PROCESS_RIGHT_SIGNAL | KERNEL_PROCESS_RIGHT_PRIORITY |
           (kernel_process_debug_surface() ? KERNEL_PROCESS_RIGHT_DEBUG : 0u);
}

static int32_t find_stack_slot(const KernelProcess *process)
{
    for (uint32_t slot = 0u; slot < KERNEL_PROCESS_THREAD_SLOT_COUNT; ++slot) {
...
static void install_thread_tls(KernelThread *thread, uint32_t base,
                               uint32_t pages)
{
...
#endif
        if (kernel_vm_adopt_page(&process->address_space, page_address,
                                 physical,
                                 KERNEL_VM_READ | KERNEL_VM_WRITE) !=
            KERNEL_VM_OK) {
    ...
    KernelPreparedThread *prepared)
{
    ...
    if (process->thread_count >= KERNEL_PROCESS_THREAD_SLOT_COUNT)
        return KERNEL_PROCESS_RESOURCE_LIMIT;
...
#endif
        vm_status = kernel_vm_adopt_page(
    ...
    KernelPreparedThread *prepared)
{
    ...
    if (prepared == NULL || prepared->process == NULL ||
        prepared->thread == NULL ||
        prepared->handle == KERNEL_HANDLE_INVALID ||
        prepared->stack_slot >= KERNEL_PROCESS_THREAD_SLOT_COUNT)
        return KERNEL_PROCESS_INVALID_ARGUMENT;
    ...
    for (uint32_t page = 0u; page < prepared->thread->tls_pages; ++page) {
        if (kernel_vm_unmap_page(
                &prepared->process->address_space,
                prepared->thread->tls_base + page * KERNEL_PAGE_SIZE) !=
            KERNEL_VM_OK)
            cleanup_failed = true;
    }
    prepared->thread->tls_base = 0u;
    prepared->thread->tls_pages = 0u;
                                         ...
                                         KernelHandle *created_handle)
{
    ...
    *created_handle = KERNEL_HANDLE_INVALID;
    ...
    if (process == NULL || thread == NULL ||
        prepared->handle == KERNEL_HANDLE_INVALID ||
        prepared->stack_slot >= KERNEL_PROCESS_THREAD_SLOT_COUNT ||
        process->thread_count >= KERNEL_PROCESS_THREAD_SLOT_COUNT ||
    ...
    KernelPreparedThread *prepared)
{
    ...
    if (process == NULL || source == NULL || prepared == NULL ||
        source->stack_slot >= KERNEL_PROCESS_THREAD_SLOT_COUNT ||
    ...
    thread->tls_pages = source->tls_pages;
    ...
    uint32_t *process_id, KernelHandle *process_handle)
{
    ...
    *process_handle = KERNEL_HANDLE_INVALID;
    result = claim_process_record(&child, &slot);
    ...
    handle_status = kernel_handle_install_cloneable(
        ...
        KERNEL_PROCESS_RIGHT_QUERY | KERNEL_PROCESS_RIGHT_TERMINATE |
            ...
            KERNEL_PROCESS_RIGHT_TRANSFER,
        child, process_handle_retain,
    ...
    result = commit_thread(&prepared, &child_thread_id,
                           &child_thread_handle);
    if (result == KERNEL_PROCESS_OK) {
        KernelThreadStatus thread_status = kernel_thread_suspend_process(slot);
...
static uint32_t segment_page_batch[KERNEL_PAGE_SIZE / sizeof(uint32_t)];
static uint8_t launch_header[KERNEL_ELF_HEADER_SIZE];
...
static union {
    char bytes[ASTRA_STARTUP_BLOCK_SIZE];
    AstraEventDrained events[ASTRA_STARTUP_BLOCK_SIZE /
                             sizeof(AstraEventDrained)];
} syscall_data;
_Static_assert(ASTRA_TEXT_COLUMNS * ASTRA_TEXT_ROWS <=
                   sizeof(syscall_data.bytes),
               "console plane exceeds serialized syscall scratch");
...
static AstraStartupCapability
    startup_granted[KERNEL_PROCESS_BOOTSTRAP_CAPABILITY_MAX];
...
static KernelProcessStatus map_segments(KernelAddressSpace *space,
                                        ...
                                        uint32_t virtual_base,
                                        bool rollback);
static KernelProcessStatus map_thread_tls(
    KernelAddressSpace *space, KernelProcess *process,
    const KernelElfTls *tls, bool consider_existing,
    uint32_t *tls_base, uint32_t *tls_pages);
    ...
    uint32_t size, const KernelElfImage *plan, uint32_t *address)
{
    ...
    *address = 0u;
    ...
    for (uint32_t index = 0u; index < plan->segment_count; ++index) {
        ...
        if (segment == NULL)
            return KERNEL_PROCESS_CORRUPT;
        if (segment->virtual_address < first_segment)
            first_segment = segment->virtual_address;
    }
...
static KernelProcessStatus
trace_exec_corruption(const KernelProcess *process, uint32_t stage,
                      uint32_t detail)
{
    (void)kernel_trace_write(
        KERNEL_TRACE_EVENT_SYSCALL_EXIT, KERNEL_TRACE_LEVEL_ERROR,
        ASTRA_SYSCALL_PROCESS_EXEC, 0xE0000000u | stage, detail,
        process != NULL ? process->id : 0u);
    return KERNEL_PROCESS_CORRUPT;
}
    ...
    KernelCpuContext **next_context)
{
    ...
    uint32_t entry_base;
    uint32_t entry_size;
    uint32_t new_tls_base = 0u;
    uint32_t new_tls_pages = 0u;
    uint32_t retired = 0u;
    uint32_t released_buffers = 0u;
    uint32_t deferred_buffers = 0u;
    ...
    status = publish_startup_block(
        ...
        request->arguments.environment_count != 0u ?
            syscall_data.bytes + request->arguments.length : NULL,
    ...
    if (host_channels_close_process(process) != ASTRA_SYSCALL_OK) {
        status = trace_exec_corruption(process, 1u, 0u);
        goto failed;
    }
    if (!display_dma_abort_owner(process->owner)) {
        status = trace_exec_corruption(process, 2u, 0u);
        goto failed;
    }
    {
        KernelBlockStatus block_status = kernel_block_revoke_owner(
            process->owner, &released_buffers, &deferred_buffers);

        if (block_status != KERNEL_BLOCK_OK) {
            status = trace_exec_corruption(process, 3u,
                                           (uint32_t)block_status);
            goto failed;
        }
    }
    if (deferred_buffers != 0u) {
        status = trace_exec_corruption(process, 4u, deferred_buffers);
        goto failed;
    }
    (void)kernel_handle_close_type(process->handles, KERNEL_OBJECT_DMA);
    if (process->dma_pages != 0u) {
        status = trace_exec_corruption(process, 5u, process->dma_pages);
        goto failed;
    }
    {
        ...
        if (area_status != KERNEL_AREA_OK) {
            status = trace_exec_corruption(process, 6u,
    ...
    }
    {
        ...
        if (thread_status != KERNEL_THREAD_OK) {
            status = trace_exec_corruption(process, 7u,
    ...
    for (uint32_t slot = 0u; slot < kernel_thread_slot_limit(); ++slot) {
        ...
        if (other->self_handle != KERNEL_HANDLE_INVALID &&
            kernel_handle_close(process->handles, other->self_handle) !=
                KERNEL_HANDLE_OK) {
            status = trace_exec_corruption(process, 8u,
                                           other->self_handle);
            goto failed;
        }
    }
...
/*
 * Populate one page and publish it. The frame's allocation reference is
 * transferred directly to the mapping, and every frame carries the process
 * owner tag so a failure anywhere unwinds through kernel_memory_release_owner()
 * with the rest of the address space.
 */
static KernelProcessStatus publish_page_tagged(
    KernelAddressSpace *space, uint32_t owner, uint32_t virtual_address,
    const uint8_t *source, uint32_t user_source, uint32_t source_size,
    uint32_t rights, KernelAllocationSite site)
{
    uint32_t physical = 0u;
    uint8_t *bytes;
    ...
    } else if (source_size != 0u) {
        kernel_bytes_copy(bytes, source, source_size);
    }

    if (kernel_vm_adopt_page(space, virtual_address, physical, rights) !=
        KERNEL_VM_OK) {
        (void)kernel_memory_release(physical, 1u, owner);
        return KERNEL_PROCESS_OUT_OF_MEMORY;
    }
                                        ...
                                        uint32_t rights)
{
    return publish_page_tagged(
        space, owner, virtual_address, source, 0u, source_size, rights,
        KERNEL_ALLOCATION_SITE_PROCESS_CODE_PAGE);
}

static KernelProcessStatus publish_user_page(
    KernelAddressSpace *space, uint32_t owner, uint32_t virtual_address,
    uint32_t user_source, uint32_t source_size, uint32_t rights)
{
    return publish_page_tagged(
        space, owner, virtual_address, NULL, user_source, source_size, rights,
        KERNEL_ALLOCATION_SITE_PROCESS_CODE_PAGE);
}

static bool release_segment_page_batch(uint32_t owner, uint32_t first,
                                       uint32_t count)
{
    bool released = true;

    for (uint32_t page = first; page < count; ++page)
        if (kernel_memory_release(segment_page_batch[page], 1u, owner) !=
            KERNEL_MEMORY_OK)
            released = false;
    return released;
}

static KernelProcessStatus publish_segment_page_batch(
    KernelAddressSpace *space, uint32_t owner,
    const KernelElfSegment *segment, const uint8_t *image,
    uint32_t user_image, uint32_t virtual_base, uint32_t first_page,
    uint32_t page_count, uint32_t rights, uint32_t *mapped_pages)
{
    *mapped_pages = 0u;
    ...
    for (uint32_t page = 0u; page < page_count; ++page) {
        uint32_t offset = (first_page + page) * KERNEL_PAGE_SIZE;
        uint32_t copy = 0u;
        uint8_t *bytes = physical_bytes(segment_page_batch[page],
                                        KERNEL_PAGE_SIZE);

        if (bytes == NULL) {
            (void)release_segment_page_batch(owner, 0u, page_count);
            return KERNEL_PROCESS_CORRUPT;
        }
        ...
        if (copy == 0u)
            continue;
        if (user_image != 0u) {
            if (kernel_copy_from_user(
                    bytes, user_image + segment->file_offset + offset,
                    copy) != KERNEL_USER_COPY_OK) {
                return release_segment_page_batch(owner, 0u, page_count) ?
                    KERNEL_PROCESS_INVALID_ARGUMENT : KERNEL_PROCESS_CORRUPT;
            }
        } else {
            kernel_bytes_copy(bytes,
                              image + segment->file_offset + offset, copy);
        }
    }

    for (uint32_t page = 0u; page < page_count; ++page) {
        uint32_t address = virtual_base + segment->virtual_address +
                           (first_page + page) * KERNEL_PAGE_SIZE;

        if (kernel_vm_adopt_page(space, address, segment_page_batch[page],
                                rights) != KERNEL_VM_OK) {
            return release_segment_page_batch(owner, page, page_count) ?
                KERNEL_PROCESS_OUT_OF_MEMORY : KERNEL_PROCESS_CORRUPT;
        }
        ++*mapped_pages;
    }
...
static bool tls_base_find(const KernelProcess *process, uint32_t span,
                          uint32_t alignment, bool consider_existing,
                          uint32_t *base)
{
    uint32_t candidate;
    uint32_t mask = alignment - 1u;

    if (process == NULL || base == NULL || span == 0u ||
        !astra_u32_is_power_of_two(alignment) ||
        !astra_u32_add_checked(KERNEL_PROCESS_TLS_BASE, mask, &candidate))
        return false;
    candidate &= ~mask;
    for (;;) {
        uint32_t candidate_end;
        uint32_t conflict_end = 0u;

        if (!astra_u32_add_checked(candidate, span, &candidate_end) ||
            candidate_end > KERNEL_PROCESS_TLS_END)
            return false;
        ...
        for (uint32_t slot = 0u; slot < kernel_thread_slot_limit(); ++slot) {
            ...
            if (thread == NULL ||
                ...
                !astra_u32_add_checked(
                    thread->tls_base,
                    thread->tls_pages * KERNEL_PAGE_SIZE, &existing_end))
                continue;
            if (candidate < existing_end &&
                thread->tls_base < candidate_end &&
                existing_end > conflict_end)
                conflict_end = existing_end;
        }
        if (conflict_end == 0u) {
            *base = candidate;
            return true;
        }
        if (!astra_u32_add_checked(conflict_end, mask, &candidate))
            return false;
        candidate &= ~mask;
    }
}
    ...
    uint32_t *tls_base, uint32_t *tls_pages)
{
    ...
    if (space == NULL || process == NULL || tls == NULL || tls_base == NULL ||
        tls_pages == NULL)
        return KERNEL_PROCESS_INVALID_ARGUMENT;
    *tls_base = 0u;
    *tls_pages = 0u;
    if (tls->memory_size == 0u)
        return KERNEL_PROCESS_OK;
    if (!astra_u32_add_checked(tls->memory_size, KERNEL_PAGE_SIZE - 1u,
                               &rounded))
        return KERNEL_PROCESS_RESOURCE_LIMIT;
    rounded &= ~(KERNEL_PAGE_SIZE - 1u);
    pages = rounded / KERNEL_PAGE_SIZE;
    alignment = tls->alignment > KERNEL_PAGE_SIZE ? tls->alignment :
                                                    KERNEL_PAGE_SIZE;
    if (!tls_base_find(process, rounded, alignment, consider_existing,
                       &base))
        return KERNEL_PROCESS_RESOURCE_LIMIT;
    for (uint32_t page = 0u; page < pages; ++page) {
        ...
        result = publish_page_tagged(
            space, process->owner, base + offset, launch_page, 0u,
            KERNEL_PAGE_SIZE, KERNEL_VM_READ | KERNEL_VM_WRITE,
...
failed:
    while (mapped != 0u) {
        --mapped;
        if (kernel_vm_unmap_page(space,
                                 base + mapped * KERNEL_PAGE_SIZE) !=
            KERNEL_VM_OK)
            return KERNEL_PROCESS_CORRUPT;
    }
    return result;
 ...
 */
/* Launch data and byte-ring transfers are serialized on this single CPU. */
                                      ...
                                      AstraLaunchArguments *arguments)
{
    ...
    kernel_bytes_clear(syscall_data.bytes, sizeof(syscall_data.bytes));
    if (user_address == 0u)
        return ASTRA_SYSCALL_OK;
    ...
    if (arguments->count != 0u) {
        copy_status = kernel_copy_from_user(
            syscall_data.bytes, arguments->argument_address,
            arguments->length);
        if (copy_status != KERNEL_USER_COPY_OK)
            return ASTRA_SYSCALL_BAD_ADDRESS;
    }
    if (arguments->environment_count != 0u) {
        copy_status = kernel_copy_from_user(
            syscall_data.bytes + arguments->length,
                                        ...
                                        bool rollback)
{
    ...
    for (uint32_t index = 0u; index < plan->segment_count; ++index) {
        ...
        for (uint32_t page = 0u; page < segment->page_count;) {
            uint32_t count = segment->page_count - page;
            uint32_t batch_mapped;
            KernelProcessStatus status;

            if (count > sizeof(segment_page_batch) /
                            sizeof(segment_page_batch[0]))
                count = sizeof(segment_page_batch) /
                        sizeof(segment_page_batch[0]);
            status = publish_segment_page_batch(
                space, owner, segment, image, user_image, virtual_base,
                page, count, rights, &batch_mapped);
            mapped += batch_mapped;
            if (status != KERNEL_PROCESS_OK) {
                failure = status;
                goto failed;
    ...
    KernelLibraryCacheEntry *entry)
{
    ...
    if (!kernel_elf_image_discard(&entry->plan))
        return KERNEL_PROCESS_CORRUPT;
...
static KernelProcessStatus attach_cached_library(
    KernelProcess *process, KernelLibraryCacheEntry *cached,
    uint32_t *base, uint32_t *span, KernelHandle *handle)
{
    KernelLibraryLoad *load = NULL;
    uint32_t load_physical = 0u;
    KernelProcessStatus status;

    if (process == NULL || cached == NULL || base == NULL || span == NULL ||
        handle == NULL)
        return KERNEL_PROCESS_INVALID_ARGUMENT;
    *base = 0u;
    *span = 0u;
    *handle = KERNEL_HANDLE_INVALID;
    if (kernel_handle_available(process->handles) == 0u)
        return KERNEL_PROCESS_RESOURCE_LIMIT;
    status = library_metadata_allocate(&load_physical, (void **)&load);
    if (status != KERNEL_PROCESS_OK)
        return status;
    load->owner = process->id;
    load->self_physical = load_physical;
    load->magic = KERNEL_LIBRARY_LOAD_MAGIC;
    load->stage = KERNEL_LIBRARY_LOAD_MAPPED;
    status = map_cached_library(process, cached, base, span);
    if (status == KERNEL_PROCESS_OK) {
        ...
        if (kernel_handle_install(
                process->handles, KERNEL_OBJECT_LIBRARY_LOAD,
                KERNEL_LIBRARY_LOAD_RIGHT, load, library_load_release,
                NULL, handle) != KERNEL_HANDLE_OK)
            status = KERNEL_PROCESS_RESOURCE_LIMIT;
    }
    if (status != KERNEL_PROCESS_OK && *handle == KERNEL_HANDLE_INVALID)
...
static void library_load_release(void *object, void *context)
{
    ...
    if (load->entry != NULL &&
        ...
        failed = true;
    if (!kernel_elf_stream_discard(&load->elf))
        failed = true;
    load->magic = 0u;
    load->self_physical = 0u;
...
static KernelProcessStatus library_load_append_page(
    KernelLibraryLoad *load, const uint8_t *bytes, uint32_t user_bytes,
    uint32_t length)
{
    KernelLibraryPageBlock *block = NULL;
    bool needs_block;
    ...
    if (!library_load_valid(load) || load->entry == NULL ||
        length > KERNEL_PAGE_SIZE ||
        (length != 0u && (bytes != NULL) == (user_bytes != 0u)))
        return KERNEL_PROCESS_INVALID_ARGUMENT;
    ...
    KernelLibraryLoad *load, uint32_t *offset, uint32_t *length)
{
    ...
    *length = 0u;
    ...
    for (;;) {
        ...
        if (load->segment > plan->segment_count)
            return KERNEL_PROCESS_CORRUPT;
        segment = kernel_elf_image_segment(plan, load->segment);
        if (segment == NULL)
            return KERNEL_PROCESS_CORRUPT;
        ...
        {
            KernelProcessStatus status =
                library_load_append_page(load, NULL, 0u, 0u);
    ...
    uint32_t length, uint32_t *next_offset, uint32_t *next_length)
{
    ...
    if (file_offset != expected_offset || length > expected_length ||
        (length < expected_length && length % KERNEL_PAGE_SIZE != 0u))
        return KERNEL_PROCESS_INVALID_ARGUMENT;
    if (load->stage == KERNEL_LIBRARY_LOAD_HEADERS) {
        KernelElfImage plan;
        ...
        if (kernel_copy_from_user(launch_page, user_bytes, length) !=
            KERNEL_USER_COPY_OK)
            return KERNEL_PROCESS_BAD_ADDRESS;
        elf_status = kernel_elf_stream_add_header(&load->elf, launch_page);
        if (elf_status != KERNEL_ELF_OK)
            return elf_status == KERNEL_ELF_OUT_OF_MEMORY ?
                KERNEL_PROCESS_OUT_OF_MEMORY :
                KERNEL_PROCESS_INVALID_ARGUMENT;
        ...
        kernel_elf_image_move(&load->entry->plan, &load->elf.plan);
    ...
    if (load->stage == KERNEL_LIBRARY_LOAD_IDENTITY) {
        ...
        if (kernel_copy_from_user(launch_page, user_bytes, length) !=
            KERNEL_USER_COPY_OK)
            return KERNEL_PROCESS_BAD_ADDRESS;
        if (!library_reference_from_record(
    ...
    if (load->stage != KERNEL_LIBRARY_LOAD_SEGMENTS || load->entry == NULL)
        return KERNEL_PROCESS_INVALID_STATE;
    if (user_bytes > UINT32_MAX - (length - 1u))
        return KERNEL_PROCESS_BAD_ADDRESS;
    for (uint32_t consumed = 0u; consumed < length;) {
        uint32_t copy = length - consumed;

        if (copy > KERNEL_PAGE_SIZE)
            copy = KERNEL_PAGE_SIZE;
        status = library_load_append_page(
            load, NULL, user_bytes + consumed, copy);
        if (status != KERNEL_PROCESS_OK)
            return status;
        ++load->page;
        consumed += copy;
    ...
    const KernelLibraryCacheEntry *right)
{
    ...
    if (left == NULL || right == NULL ||
        ...
        !kernel_elf_image_equal(&left->plan, &right->plan) ||
...
static KernelElfStatus accept_user_elf(
    uint32_t user_image, uint32_t image_size, const KernelElfLimits *limits,
    KernelElfRole role, KernelElfImage *plan)
{
    ...
    if (user_image == 0u || image_size < KERNEL_ELF_HEADER_SIZE ||
        limits == NULL || plan == NULL)
        return KERNEL_ELF_INVALID_ARGUMENT;
    kernel_bytes_clear(plan, sizeof(*plan));
    kernel_bytes_clear(launch_header, sizeof(launch_header));
    if (kernel_copy_from_user(launch_header, user_image,
                              KERNEL_ELF_HEADER_SIZE) != KERNEL_USER_COPY_OK)
        return KERNEL_ELF_INVALID_ARGUMENT;
    status = role == KERNEL_ELF_EXECUTABLE ?
        kernel_elf_stream_begin(launch_header, image_size, limits, &stream) :
        kernel_elf_stream_begin_interpreter(launch_header, image_size, limits,
                                            &stream);
    while (status == KERNEL_ELF_OK) {
        status = kernel_elf_stream_next_header(&stream, &offset, &length);
        if (status != KERNEL_ELF_OK || length == 0u)
            break;
        if (offset > UINT32_MAX - user_image ||
            length > sizeof(launch_header) ||
            kernel_copy_from_user(launch_header, user_image + offset,
                                  length) != KERNEL_USER_COPY_OK) {
            kernel_elf_stream_discard(&stream);
            return KERNEL_ELF_INVALID_ARGUMENT;
        }
        status = kernel_elf_stream_add_header(&stream, launch_header);
    }
                                             ...
                                             KernelElfImage *plan)
{
    if (plan == NULL || (image == NULL) == (user_image == 0u) ||
        ...
        return KERNEL_PROCESS_INVALID_ARGUMENT;
    if (user_image != 0u) {
        KernelElfStatus elf_status = accept_user_elf(
            user_image, image_size, &executable_limits,
            KERNEL_ELF_EXECUTABLE, plan);
    ...
    {
        KernelElfStatus elf_status = kernel_elf_accept(
            image, image_size, &executable_limits, plan);
                                  ...
                                  KernelInitialImage *initial_image)
{
    ...
    const KernelElfSegment *first;

    if (program == NULL || interpreter == NULL || initial_image == NULL ||
        interpreter->segment_count == 0u)
        return false;
    first = kernel_elf_image_segment(interpreter, 0u);
    if (first == NULL || first->virtual_address != 0u ||
        first->file_offset != 0u ||
        first->file_size < KERNEL_ELF_HEADER_SIZE)
        return false;
    for (uint32_t index = 0u; index < interpreter->segment_count; ++index) {
        const KernelElfSegment *segment =
            kernel_elf_image_segment(interpreter, index);
        if (segment == NULL)
            return false;
    ...
    for (;;) {
        ...
        for (uint32_t index = 0u; index < program->segment_count; ++index) {
            const KernelElfSegment *segment =
                kernel_elf_image_segment(program, index);
            if (segment == NULL)
                return false;
    ...
    KernelProcess **created, KernelPreparedThread *prepared)
{
    ...
    *created = NULL;
    kernel_bytes_clear(prepared, sizeof(*prepared));
    kernel_bytes_clear(startup_granted, sizeof(startup_granted));
    result = claim_process_record(&process, &slot);
    if (result != KERNEL_PROCESS_OK)
        return result;
    if (source_table == NULL ||
    ...
    result = grant_bootstrap_capabilities(process, source_table, capabilities,
                                          capability_count, startup_granted);
    if (result != KERNEL_PROCESS_OK)
        goto failed;
    result = publish_startup_block(
        &process->address_space, process->owner, self_handle,
        prepared->handle, startup_granted, capability_count, arguments,
...
static void executable_load_release(void *object, void *context)
{
    ...
    if (!kernel_elf_stream_discard(&load->elf) ||
        !kernel_elf_stream_discard(&load->interpreter))
        process_pool_corrupt = 1u;
    ...
    KernelExecutableLoad *load, uint32_t *offset, uint32_t *length)
{
    ...
    *length = 0u;
    ...
    for (;;) {
        ...
        {
        const KernelElfSegment *segment =
            kernel_elf_image_segment(plan, load->segment);
        if (segment == NULL)
            return KERNEL_PROCESS_CORRUPT;
    ...
    uint32_t length, uint32_t *next_offset, uint32_t *next_length)
{
    ...
    if (load->stage == KERNEL_EXECUTABLE_LOAD_PROGRAM_HEADERS ||
        ...
        if (length != KERNEL_ELF_PHENTSIZE ||
            kernel_copy_from_user(launch_header, user_bytes, length) !=
                KERNEL_USER_COPY_OK)
            return KERNEL_PROCESS_BAD_ADDRESS;
        elf_status = kernel_elf_stream_add_header(stream, launch_header);
        if (elf_status != KERNEL_ELF_OK)
            return elf_status == KERNEL_ELF_OUT_OF_MEMORY ?
                KERNEL_PROCESS_OUT_OF_MEMORY :
                KERNEL_PROCESS_INVALID_ARGUMENT;
        ...
        return KERNEL_PROCESS_INVALID_STATE;
    {
        ...
        while (consumed < length) {
            uint32_t page_offset =
                (uint32_t)load->page * KERNEL_PAGE_SIZE;
            uint32_t copy = length - consumed;
    ...
    uint32_t *child_id)
{
    ...
    *child_id = 0u;
    ...
    if (status != KERNEL_PROCESS_OK)
        return status;
    install_thread_tls(prepared.thread, prepared.thread->tls_base,
                       prepared.thread->tls_pages);
    load->process = NULL;
    ...
    handle_status = kernel_handle_install_cloneable(
        ...
        KERNEL_PROCESS_RIGHT_QUERY | KERNEL_PROCESS_RIGHT_WAIT |
            KERNEL_PROCESS_RIGHT_TERMINATE | KERNEL_PROCESS_RIGHT_SIGNAL |
            KERNEL_PROCESS_RIGHT_TRANSFER | KERNEL_PROCESS_RIGHT_PRIORITY,
        child, process_handle_retain, process_handle_release, NULL,
        child_handle);
    if (handle_status != KERNEL_HANDLE_OK) {
        process_handle_release(child, NULL);
        destroy_prepared_executable(child, &prepared);
    ...
    uint32_t *process_id)
{
    ...
    *process_id = 0u;
    ...
    if (result != KERNEL_PROCESS_OK) {
        if (!kernel_elf_image_discard(&plan))
            return KERNEL_PROCESS_CORRUPT;
        return result;
    }
    ...
    uint32_t rights, KernelHandle *handle)
{
    ...
    *handle = KERNEL_HANDLE_INVALID;
    ...
    handle_status = kernel_handle_install_cloneable(
        ...
        process_handle_retain, process_handle_release, NULL, handle);
    if (handle_status == KERNEL_HANDLE_OK)
    ...
    uint32_t process_id, uint32_t irq_source_mask)
{
    ...
    if (process == NULL ||
        ...
        scheduler_started != 0u)
        return KERNEL_PROCESS_INVALID_ARGUMENT;
    process->qualification_authorized_sources = irq_source_mask;
    process->qualification_completed_sources = 0u;
    process->qualification_authorized = 1u;
    return KERNEL_PROCESS_OK;
}
                                         ...
                                         uint32_t *completed_sources)
{
    ...
    if (authorized_sources == NULL || completed_sources == NULL)
        return false;
    process = find_process_by_id(process_id);
    if (process == NULL || process->qualification_authorized == 0u)
        return false;
    *authorized_sources = process->qualification_authorized_sources;
    *completed_sources = process->qualification_completed_sources;
    return true;
}

static bool qualification_source_allowed(
    const KernelProcess *process, uint32_t source)
{
    return process != NULL && process->qualification_authorized != 0u &&
           source < KERNEL_IRQ_SOURCE_COUNT &&
           (process->qualification_authorized_sources & (1u << source)) != 0u;
}
    ...
    bool *handled)
{
    ...
    *handled = (command & KERNEL_QUALIFICATION_COMMAND_MASK) ==
    ...
    if (!*handled)
        return KERNEL_PROCESS_OK;
    if (process->qualification_authorized == 0u) {
    ...
    case KERNEL_QUALIFICATION_COMMAND_QUERY_IRQS:
        thread->context.data[1] = process->qualification_authorized_sources;
        return KERNEL_PROCESS_OK;
    case KERNEL_QUALIFICATION_COMMAND_BIND_IRQ: {
        ...
        if (!qualification_source_allowed(process, source) ||
    ...
    case KERNEL_QUALIFICATION_COMMAND_COMPLETE_IRQS:
         ...
         */
        if (source == 0u ||
            (source & ~process->qualification_authorized_sources) != 0u) {
            *syscall_result = ASTRA_SYSCALL_INVALID_ARGUMENT;
            return KERNEL_PROCESS_OK;
        }
        process->qualification_completed_sources = source;
        return KERNEL_PROCESS_OK;
    default:
        *syscall_result = ASTRA_SYSCALL_INVALID_ARGUMENT;
        return KERNEL_PROCESS_OK;
...
KernelCpuContext *kernel_process_current_context(void)
{
    ...
    if ((quantum_preempt_pending != 0u ||
         ...
         ready_thread_outranks(current_thread)) &&
            schedule_pending(&next, false) != KERNEL_PROCESS_OK)
        return NULL;
    return runtime_resume(current_thread);
}

static KernelCpuContext *resume_idle(bool from_worker)
{
    KernelCpuContext *next;
    uint8_t ready_priority;

    if (scheduler_initialized == 0u || scheduler_started == 0u ||
        current_thread != NULL || worker_active != (from_worker ? 1u : 0u) ||
...
KernelCpuContext *kernel_process_worker_resume(void)
{
    ...
    if (worker_active == 0u)
        return NULL;
    /* Work serviced while no process ran may have made a thread ready. */
    if (current_thread == NULL) {
        next = resume_idle(true);
        if (next == NULL)
            return NULL;
        worker_active = 0u;
        scheduler_start_quantum();
                                            ...
                                            KernelCpuContext **next_context)
{
    ...
    if (next_context == NULL)
        return KERNEL_PROCESS_INVALID_ARGUMENT;
    *next_context = NULL;
    status = capture_current(registers, user_stack, raw_frame, UINT16_MAX,
                             &current, &previous);
    if (status != KERNEL_PROCESS_OK)
        return status;
    (void)current;
    ...
    if (quantum_preempt_pending != 0u ||
        ...
        return schedule_pending(next_context, false);
...
KernelProcessStatus kernel_process_on_supervisor_timer(void)
{
    ...
    if (scheduler_initialized == 0u)
        return KERNEL_PROCESS_OK;
    /*
     * A supervisor frame means the timer interrupted kernel work.  Scheduler
     * queues may be between stores -- in particular deadline_remove() changes
     * the heap count before replacing its root -- so re-entering expiration
     * here corrupts otherwise valid state.  The scheduler quantum is already
     * its maximum dispatch latency; re-arm that tick and let the next user
     * timer process every due deadline from a stable boundary.
     *
     * With no current thread the kernel is genuinely idle, not nested inside
     * one, and expiration is what makes the last sleeping thread runnable.
     */
    if (current_thread != NULL) {
        ++scheduler_stats.supervisor_timer_deferrals;
        kernel_platform_timer_arm(scheduler_quantum_cycles);
        ++scheduler_stats.timer_rearms;
        return KERNEL_PROCESS_OK;
    }
    now = scheduler_cycles();
    status = scheduler_expire_due(now, NULL, NULL);
    if (status != KERNEL_PROCESS_OK)
        return status;
    ...
    uint32_t *syscall_result)
{
    ...
    KernelObjectType first_type = KERNEL_OBJECT_NONE;
    ...
    *blocked = false;
    ...
    for (uint32_t index = 0u; index < count; ++index) {
        ...
        if (handle_status != KERNEL_HANDLE_OK || object == NULL)
            return KERNEL_PROCESS_CORRUPT;
        if (type != KERNEL_OBJECT_SYNC && type != KERNEL_OBJECT_TIMER &&
            type != KERNEL_OBJECT_THREAD && type != KERNEL_OBJECT_PROCESS &&
            type != KERNEL_OBJECT_PORT_SEND &&
            type != KERNEL_OBJECT_PORT_RECEIVE &&
            type != KERNEL_OBJECT_RING_PRODUCER &&
            type != KERNEL_OBJECT_RING_CONSUMER &&
            type != KERNEL_OBJECT_IRQ) {
            *syscall_result = ASTRA_SYSCALL_INVALID_HANDLE;
            return KERNEL_PROCESS_OK;
        }
        if (type == KERNEL_OBJECT_THREAD && object == thread) {
            *syscall_result = ASTRA_SYSCALL_INVALID_ARGUMENT;
            return KERNEL_PROCESS_OK;
        }
        if (type == KERNEL_OBJECT_PROCESS && object == process) {
            *syscall_result = ASTRA_SYSCALL_INVALID_ARGUMENT;
            return KERNEL_PROCESS_OK;
        }
        if (index == 0u) {
    ...
    for (uint32_t index = 0u; index < count; ++index) {
        ...
        if (index == 0u) {
            type = first_type;
            object = first_object;
        } else if (kernel_handle_lookup_any(
                       process->handles, handles[index], ASTRA_RIGHT_WAIT,
                       &type, &object) != KERNEL_HANDLE_OK || object == NULL) {
            return KERNEL_PROCESS_CORRUPT;
        }

        if (type == KERNEL_OBJECT_SYNC || type == KERNEL_OBJECT_TIMER) {
            KernelSyncObject *sync = object;
            KernelSyncStatus sync_status =
                kernel_sync_prepare_wait(sync, &specs[index]);
            ...
            if (sync_status == KERNEL_SYNC_CLOSED) {
                wait_result = kernel_sync_terminal_result(sync);
            ...
            if (sync_status != KERNEL_SYNC_BLOCKED)
                return sync_status == KERNEL_SYNC_INVALID_ARGUMENT ||
                    ...
                    KERNEL_PROCESS_INVALID_STATE : KERNEL_PROCESS_CORRUPT;
        } else if (type == KERNEL_OBJECT_PORT_SEND ||
                   type == KERNEL_OBJECT_PORT_RECEIVE) {
            ...
            KernelPortStatus port_status = after_failed_send ?
                kernel_port_prepare_wait_after(
                    object, endpoint, port_probe_sequence,
                    &specs[index]) :
                kernel_port_prepare_wait(
                    object, endpoint, &specs[index]);
            ...
            if (port_status != KERNEL_PORT_WOULD_BLOCK)
                return port_status == KERNEL_PORT_INVALID_ARGUMENT ||
                    ...
                    KERNEL_PROCESS_INVALID_STATE : KERNEL_PROCESS_CORRUPT;
        } else if (type == KERNEL_OBJECT_RING_PRODUCER ||
                   type == KERNEL_OBJECT_RING_CONSUMER) {
            KernelRingStatus ring_status = kernel_ring_prepare_wait(
                object, ring_endpoint_for_type(type), &specs[index]);
        ...
        } else if (type == KERNEL_OBJECT_IRQ) {
            ...
            if (irq_status != KERNEL_IRQ_WOULD_BLOCK)
                return irq_status == KERNEL_IRQ_INVALID_ARGUMENT ||
                    ...
                    KERNEL_PROCESS_INVALID_STATE : KERNEL_PROCESS_CORRUPT;
        } else if (type == KERNEL_OBJECT_THREAD) {
            bool ready;
            KernelThreadStatus thread_status =
                kernel_thread_prepare_death_wait(
                    object, thread, &specs[index], &ready,
                    &wait_result, &detail);
            ...
            KernelProcessStatus process_status =
                prepare_process_death_wait(
                    object, process, &specs[index], &ready,
                    &wait_result, &detail);
    ...
    if (multiple) {
        ...
        thread->context.data[2] = 0u;
    } else if (first_type == KERNEL_OBJECT_THREAD ||
               first_type == KERNEL_OBJECT_PROCESS) {
        thread->context.data[1] = 0u;
    }
    ...
    if (thread_status == KERNEL_THREAD_OUT_OF_MEMORY) {
        *syscall_result = ASTRA_SYSCALL_OUT_OF_MEMORY;
        return KERNEL_PROCESS_OK;
    }
    if (thread_status != KERNEL_THREAD_OK)
    ...
    for (uint32_t index = 0u; index < count; ++index) {
        ...
        if (type == KERNEL_OBJECT_SYNC || type == KERNEL_OBJECT_TIMER) {
            ...
            if (kernel_port_commit_wait(object, endpoint) !=
                KERNEL_PORT_OK)
                return KERNEL_PROCESS_CORRUPT;
        } else if (type == KERNEL_OBJECT_RING_PRODUCER ||
                   type == KERNEL_OBJECT_RING_CONSUMER) {
            if (kernel_ring_commit_wait(object,
                                        ring_endpoint_for_type(type)) !=
                KERNEL_RING_OK)
                return KERNEL_PROCESS_CORRUPT;
        } else if (type == KERNEL_OBJECT_IRQ) {
            if (kernel_irq_commit_wait(object) != KERNEL_IRQ_OK)
                return KERNEL_PROCESS_CORRUPT;
        } else if (type == KERNEL_OBJECT_THREAD) {
            if (kernel_thread_commit_death_wait(object) !=
                KERNEL_THREAD_OK)
                return KERNEL_PROCESS_CORRUPT;
            ++scheduler_stats.thread_death_waits;
        } else {
            if (commit_process_death_wait(object) !=
                KERNEL_PROCESS_OK)
                return KERNEL_PROCESS_CORRUPT;
        }
    }
                                        ...
                                        uint32_t *result)
{
    ...
    if (syscall == ASTRA_SYSCALL_PORT_SEND_TRY) {
        ...
        if (message_size < ASTRA_MESSAGE_HEADER_SIZE ||
            ...
            (handle_count != 0u &&
             (user_handles & (ASTRA_SCALAR_ALIGNMENT - 1u)) != 0u)) {
            *result = ASTRA_SYSCALL_INVALID_ARGUMENT;
            return KERNEL_PROCESS_OK;
        }
        copy_status = kernel_copy_from_user(
                              ...
                              AstraProcessInfo *info)
{
    ...
    uint64_t now_cycles;
    uint64_t runtime_cycles = process->runtime_cycles;
    ...
    now_cycles = ((uint64_t)now.high << 32) | now.low;
    if (runtime_active != 0u && current_thread != NULL &&
        current_thread->process_slot == slot)
        runtime_cycles += (uint32_t)(now.low - runtime_started_cycles);
    kernel_bytes_clear(info, sizeof(*info));
    info->size = sizeof(*info);
    info->id = process->id;
    ...
    info->handle_references = process->handle_references;
    info->process_state = process->process_state;
    info->thread_count = process->thread_count;
    info->live_threads = process->live_threads;
    info->default_priority = process->default_priority;
    info->priority_ceiling = process->priority_ceiling;
    info->exit_reason = process->exit_reason;
    info->thread_state = (uint8_t)
        kernel_thread_process_representative_state(slot);
    info->suspended = process->suspended;
    info->runtime_ns = kernel_platform_cycles_to_ns(runtime_cycles);
    info->elapsed_ns = kernel_platform_cycles_to_ns(
        now_cycles - process->started_cycles);
    info->fault_pc = process->fault_pc;
    info->fault_address = process->fault_address;
    info->fault_vector = process->fault_vector;
...
{
    uint64_t runtime_cycles = thread->runtime_cycles;

    if (runtime_active != 0u && current_thread == thread)
        runtime_cycles += (uint32_t)(kernel_platform_cpu_cycles_low() -
                                     runtime_started_cycles);
    kernel_bytes_clear(info, sizeof(*info));
    info->size = sizeof(*info);
    info->id = thread->id;
                                 ...
                                 const KernelLibraryCacheEntry *entry)
{
    ...
    for (uint32_t segment_index = 0u;
        ...
        const KernelElfSegment *segment =
            kernel_elf_image_segment(&entry->plan, segment_index);
...
static bool library_mapping_count(const KernelLibraryCacheEntry *entry,
                                  uint32_t *count)
{
    uint32_t mappings = 0u;
                                  ...
                                  AstraProcLibrarySnapshot *record)
{
    ...
    for (uint32_t process_slot = 0u;
        ...
        if (process == NULL ||
            ...
            !process_maps_library(process, entry))
            continue;
        if (process->id == 0u || process->id > ASTRA_PROCESS_ID_MAX ||
            record->mapping_count == UINT32_MAX)
            return false;
        if (record->mapping_count == mapping_ordinal)
    ...
    return mapping_ordinal < (record->mapping_count == 0u ? 1u :
                                                        record->mapping_count);
}
...
{
    enum { TEST_LIBRARY_MAPPING_COUNT = 40u };
    KernelLibraryCacheEntry *entry = NULL;
    KernelLibraryPageBlock *block = NULL;
    KernelProcess *test_processes[TEST_LIBRARY_MAPPING_COUNT] = {0};
    AstraProcLibrarySnapshot record;
    uint32_t block_physical = 0u;
    uint32_t entry_physical = 0u;
    uint32_t physical = 0u;
    ...
    for (uint32_t index = 0u;
         index < TEST_LIBRARY_MAPPING_COUNT; ++index) {
        if (claim_process_record(&test_processes[index], &slot) !=
            KERNEL_PROCESS_OK)
            goto finished;
        test_processes[index]->id = index + 1u;
        test_processes[index]->process_state = KERNEL_PROCESS_RUNNING;
        if (kernel_vm_create_address_space(
                test_processes[index]->owner,
                &test_processes[index]->address_space) != KERNEL_VM_OK)
            goto finished;
    }
    if (library_metadata_allocate(&entry_physical, (void **)&entry) !=
            KERNEL_PROCESS_OK ||
    ...
    entry->reference.patch = 4u;
    entry->reference.abi_major = 2u;
    kernel_bytes_copy(entry->reference.name, "font.library", 13u);
    for (uint32_t index = 0u;
         index < TEST_LIBRARY_MAPPING_COUNT; ++index) {
        if (kernel_vm_map_shared_page(
                &test_processes[index]->address_space,
                entry->base, physical, LIBRARY_CACHE_OWNER,
                KERNEL_VM_READ) != KERNEL_VM_OK)
            goto finished;
    }
    passed = library_snapshot_fill(
                 entry, TEST_LIBRARY_MAPPING_COUNT - 1u, &record) &&
             ...
             record.mapped_bytes ==
                 TEST_LIBRARY_MAPPING_COUNT * KERNEL_PAGE_SIZE &&
             record.mapping_count == TEST_LIBRARY_MAPPING_COUNT &&
             record.reference_count == TEST_LIBRARY_MAPPING_COUNT + 1u &&
             record.process_id == TEST_LIBRARY_MAPPING_COUNT &&
             !library_snapshot_fill(entry, TEST_LIBRARY_MAPPING_COUNT,
                                    &record);

finished:
    for (uint32_t index = 0u;
         index < TEST_LIBRARY_MAPPING_COUNT; ++index) {
        KernelProcess *process = test_processes[index];
...
static KernelProcessStatus snapshot_authorize(KernelProcess *current,
                                              uint32_t handle,
                                              uint32_t *result)
{
    KernelProcess *observer = NULL;
    KernelHandleStatus handle_status;

    handle_status = kernel_handle_lookup(
        current->handles, handle, KERNEL_OBJECT_PROCESS,
        KERNEL_PROCESS_RIGHT_QUERY, (void **)&observer);
    if (handle_status == KERNEL_HANDLE_INVALID_HANDLE ||
        handle_status == KERNEL_HANDLE_TYPE_MISMATCH) {
        *result = ASTRA_SYSCALL_INVALID_HANDLE;
        return KERNEL_PROCESS_OK;
    ...
    if (handle_status != KERNEL_HANDLE_OK || observer == NULL)
        return KERNEL_PROCESS_CORRUPT;
    if (observer != current || current->id != initial_image_process_id)
        *result = ASTRA_SYSCALL_ACCESS_DENIED;
    return KERNEL_PROCESS_OK;
}
                                              ...
                                              KernelCpuContext **next_context)
{
    ...
    if (next_context == NULL)
        return KERNEL_PROCESS_INVALID_ARGUMENT;
    *next_context = NULL;
    status = capture_current(registers, user_stack, raw_frame,
                             ASTRA_SYSCALL_VECTOR, &current, &thread);
    if (status != KERNEL_PROCESS_OK)
        return status;
    ...
    case ASTRA_SYSCALL_LIBRARY_ATTACH: {
        ...
        library_status = attach_cached_library(
            current, cached, &base, &span, &load_handle);
    ...
    case ASTRA_SYSCALL_LIBRARY_ATTACH_RESIDENT: {
        char identity[ASTRA_LIBRARY_NAME_MAX];
        KernelLibraryCacheEntry *cached = NULL;
        KernelHandle load_handle = KERNEL_HANDLE_INVALID;
        uint32_t physical;
        uint32_t base = 0u;
        ...
        if (kernel_copy_from_user(identity, thread->context.data[1],
                                  sizeof(identity)) != KERNEL_USER_COPY_OK ||
            !library_name_valid(identity)) {
            result = ASTRA_SYSCALL_INVALID_ARGUMENT;
            break;
        }
        physical = library_cache_head;
        while (physical != 0u) {
            ...
            if (candidate == NULL)
                return KERNEL_PROCESS_CORRUPT;
            physical = candidate->next_physical;
            if (!kernel_bytes_equal(candidate->reference.name, identity,
                                    sizeof(identity)))
                continue;
            if (cached != NULL) {
                cached = NULL;
    ...
    case ASTRA_SYSCALL_CONSOLE_CURSOR: {
        ...
        }
        {
            ...
            if (columns == 0u || rows == 0u || columns > UINT32_MAX / rows ||
                (plane = columns * rows) > sizeof(syscall_data.bytes))
                return KERNEL_PROCESS_CORRUPT;
            if (count == 0u || cell > plane || plane - cell < count) {
                result = ASTRA_SYSCALL_INVALID_ARGUMENT;
                break;
            }
            copy_status = kernel_copy_from_user(syscall_data.bytes, user_cells,
                                                count);
            if (copy_status == KERNEL_USER_COPY_BAD_ADDRESS ||
                copy_status == KERNEL_USER_COPY_INVALID_ARGUMENT) {
                result = ASTRA_SYSCALL_BAD_ADDRESS;
    ...
    case ASTRA_SYSCALL_PROCESS_LOAD_CREATE: {
        ...
        load_status = executable_load_create_process(
            ...
            arguments.environment_count != 0u ?
                syscall_data.bytes + arguments.length : NULL,
            &next_offset, &next_length);
        if (!executable_status_to_syscall(load_status, &result))
            return KERNEL_PROCESS_CORRUPT;
        if (load_status == KERNEL_PROCESS_OK) {
    ...
    case ASTRA_SYSCALL_TRACE_READ: {
         ...
         */
        AstraEventDrained *events = syscall_data.events;
        ...
        if (capacity > UINT32_MAX / ASTRA_EVENT_DRAINED_SIZE) {
            result = ASTRA_SYSCALL_BAD_ADDRESS;
            break;
        }
        bytes = capacity * ASTRA_EVENT_DRAINED_SIZE;
        if (!kernel_user_copy_range_valid(user_buffer, bytes)) {
            result = ASTRA_SYSCALL_BAD_ADDRESS;
            break;
        }
        ...
        while (copied < capacity) {
            uint32_t chunk_capacity =
                (uint32_t)(sizeof(syscall_data.events) / sizeof(*events));
            uint32_t chunk_copied;
            uint32_t chunk_lost = 0u;

            if (chunk_capacity > capacity - copied)
                chunk_capacity = capacity - copied;
            chunk_copied = kernel_trace_drain_user(
                cursor, events, chunk_capacity, &cursor, &chunk_lost);
            if (!astra_u32_add_checked(lost, chunk_lost, &lost))
                lost = UINT32_MAX;
            if (chunk_copied == 0u)
                break;
            copy_status = kernel_copy_to_user(
                user_buffer + copied * ASTRA_EVENT_DRAINED_SIZE, events,
                chunk_copied * ASTRA_EVENT_DRAINED_SIZE);
            if (copy_status == KERNEL_USER_COPY_BAD_ADDRESS ||
                copy_status == KERNEL_USER_COPY_INVALID_ARGUMENT) {
                result = ASTRA_SYSCALL_BAD_ADDRESS;
                break;
            ...
            if (copy_status != KERNEL_USER_COPY_OK)
                return KERNEL_PROCESS_CORRUPT;
            copied += chunk_copied;
            if (chunk_copied < chunk_capacity)
                break;
        }
        if (result != ASTRA_SYSCALL_OK)
            break;
    ...
    case ASTRA_SYSCALL_THREAD_CREATE: {
        ...
        kernel_bytes_clear(&prepared_thread, sizeof(prepared_thread));
        ...
        } else if (prepared_thread.thread != NULL) {
            kernel_enable_interrupts();
            (void)abort_prepared_thread(&prepared_thread);
            kernel_disable_interrupts();
        }
    ...
    case ASTRA_SYSCALL_VM_PRIVATE_COMMIT: {
        KernelVmStatus vm_status = kernel_vm_private_commit_range(
            &current->address_space, thread->context.data[1],
            thread->context.data[2], true);

        if (vm_status != KERNEL_VM_OK &&
            !vm_status_to_syscall(vm_status, &result))
            return KERNEL_PROCESS_CORRUPT;
        break;
    }
    ...
    case ASTRA_SYSCALL_THREAD_SLEEP: {
        uint64_t deadline;
        uint64_t now;
        uint64_t time_ns = ((uint64_t)thread->context.data[1] << 32) |
                           thread->context.data[2];
        uint32_t flags = thread->context.data[3];
        KernelThreadStatus thread_status;

        now = scheduler_cycles();
        if ((flags & ~ASTRA_THREAD_SLEEP_FLAG_MASK) != 0u ||
            ((flags & ASTRA_THREAD_SLEEP_RELATIVE) != 0u ?
                 ((time_ns & (1ull << 63)) != 0u ||
                  !kernel_platform_deadline_to_cycles((int64_t)time_ns,
                                                       &deadline) ||
                  UINT64_MAX - now < deadline) :
                 !decode_wait_deadline(thread->context.data[1],
                                       thread->context.data[2], &deadline))) {
            result = ASTRA_SYSCALL_INVALID_ARGUMENT;
            break;
        }
        ...
        thread_status = kernel_thread_block_until(
            thread, &sleep_waiters,
            kernel_thread_wait_queue_sequence(&sleep_waiters), now,
    ...
    case ASTRA_SYSCALL_RING_READ_TRY: {
        ...
        if (handle_status != KERNEL_HANDLE_OK || ring == NULL)
            return KERNEL_PROCESS_CORRUPT;
        if (!kernel_user_copy_range_valid(thread->context.data[2], capacity)) {
            result = ASTRA_SYSCALL_BAD_ADDRESS;
            break;
        }
        started = kernel_performance_cycles_low();
        while (copied < capacity) {
            uint32_t chunk = capacity - copied;
            uint32_t chunk_copied = 0u;

            if (chunk > sizeof(syscall_data.bytes))
                chunk = sizeof(syscall_data.bytes);
            ring_status = kernel_ring_copy_peek(
                ring, copied, syscall_data.bytes, chunk, &chunk_copied);
            if (ring_status != KERNEL_RING_OK) {
            ...
            copy_status = kernel_copy_to_user(
                thread->context.data[2] + copied, syscall_data.bytes,
                chunk_copied);
            if (copy_status != KERNEL_USER_COPY_OK) {
                kernel_ring_record_copy_cycles(
                    kernel_performance_cycles_low() - started, 0u);
    ...
    case ASTRA_SYSCALL_RING_WRITE_TRY: {
        ...
        if (handle_status != KERNEL_HANDLE_OK || ring == NULL)
            return KERNEL_PROCESS_CORRUPT;
        if (!kernel_user_copy_range_valid(thread->context.data[2],
                                          thread->context.data[3])) {
            result = ASTRA_SYSCALL_BAD_ADDRESS;
            break;
        }
        started = kernel_performance_cycles_low();
        ring_status = kernel_ring_copy_write_prepare(
            ring, thread->context.data[3],
        ...
        for (uint32_t offset = 0u; offset < written;) {
            uint32_t chunk = written - offset;

            if (chunk > sizeof(syscall_data.bytes))
                chunk = sizeof(syscall_data.bytes);
            copy_status = kernel_copy_from_user(
                syscall_data.bytes, thread->context.data[2] + offset, chunk);
            if (copy_status != KERNEL_USER_COPY_OK) {
                kernel_ring_record_copy_cycles(
                    kernel_performance_cycles_low() - started, 0u);
                result = copy_status == KERNEL_USER_COPY_BAD_ADDRESS ||
            ...
            ring_status = kernel_ring_copy_write_at(
                ring, offset, syscall_data.bytes, chunk);
            if (ring_status != KERNEL_RING_OK)
                return KERNEL_PROCESS_CORRUPT;
            offset += chunk;
        }
        if (result != ASTRA_SYSCALL_OK)
            break;
        ring_status = kernel_ring_copy_write_commit(ring, written, &woken);
        if (ring_status != KERNEL_RING_OK)
            return KERNEL_PROCESS_CORRUPT;
        kernel_ring_record_copy_cycles(
    ...
    case ASTRA_SYSCALL_INPUT_READ_TRY: {
        ...
        if (capacity > ASTRA_INPUT_FIFO_CAPACITY)
            capacity = ASTRA_INPUT_FIFO_CAPACITY;
        device_status = kernel_device_query(lease, &snapshot);
        if (!device_status_to_syscall(device_status, &result))
            return KERNEL_PROCESS_CORRUPT;
        if (device_status != KERNEL_DEVICE_OK)
    ...
    case ASTRA_SYSCALL_HOST_QUERY:
    case ASTRA_SYSCALL_HOST_EXECUTE:
    case ASTRA_SYSCALL_HOST_CHANNEL_OPEN:
    case ASTRA_SYSCALL_HOST_CHANNEL_CLOSE: {
        KernelDeviceLease *lease = NULL;
        KernelDeviceSnapshot snapshot;
        KernelHandleStatus handle_status;
        uint32_t required_rights = syscall == ASTRA_SYSCALL_HOST_QUERY ?
            KERNEL_DEVICE_RIGHT_QUERY : KERNEL_DEVICE_RIGHT_EXECUTE;
        ...
        if (snapshot.lease_state != ASTRA_DEVICE_LEASE_ACTIVE &&
            syscall != ASTRA_SYSCALL_HOST_CHANNEL_CLOSE) {
            result = ASTRA_SYSCALL_PEER_DEAD;
            break;
        }
    ...
    case ASTRA_SYSCALL_HOST_CHANNEL_WAIT: {
        ...
        if (!decode_wait_deadline(thread->context.data[3],
                                  thread->context.data[4],
                                  &deadline_cycles)) {
            result = ASTRA_SYSCALL_INVALID_ARGUMENT;
            break;
        }
        status = host_channel_wait(
            current, thread, thread->context.data[2], scheduler_cycles(),
    ...
    case ASTRA_SYSCALL_LIBRARY_SNAPSHOT: {
        ...
        while (physical != 0u) {
            ...
            if (entry == NULL ||
                !library_mapping_count(entry, &mappings))
                return KERNEL_PROCESS_CORRUPT;
            rows = mappings == 0u ? 1u : mappings;
            if (records > UINT32_MAX - rows)
                return KERNEL_PROCESS_CORRUPT;
            records += rows;
            physical = entry->next_physical;
        }
        ...
        while (physical != 0u && index < moved) {
            ...
            if (entry == NULL ||
                ...
                return KERNEL_PROCESS_CORRUPT;
            rows = mappings == 0u ? 1u : mappings;
            if (start >= ordinal + rows) {
                ordinal += rows;
                physical = entry->next_physical;
                continue;
            }
            uint32_t mapping = start > ordinal ? start - ordinal : 0u;

            while (mapping < rows && index < moved) {
                uint32_t destination;

                if (index > (UINT32_MAX - thread->context.data[2]) /
                            (uint32_t)sizeof(*record) ||
                    !library_snapshot_fill(entry, mapping, record))
                    return KERNEL_PROCESS_CORRUPT;
    ...
    case ASTRA_SYSCALL_PROCESS_SIGNAL:
    case ASTRA_SYSCALL_PROCESS_TERMINATE:
    case ASTRA_SYSCALL_PROCESS_SUSPEND:
    case ASTRA_SYSCALL_PROCESS_RESUME: {
        KernelProcess *target = NULL;
        KernelHandleStatus handle_status;
        uint32_t signal = thread->context.data[2];
        uint32_t required_right =
            syscall == ASTRA_SYSCALL_PROCESS_TERMINATE ?
                KERNEL_PROCESS_RIGHT_TERMINATE :
                KERNEL_PROCESS_RIGHT_SIGNAL;
        bool carries_signal = syscall == ASTRA_SYSCALL_PROCESS_SIGNAL ||
                              syscall == ASTRA_SYSCALL_PROCESS_TERMINATE;

        if (carries_signal && (signal == 0u || signal >= 32u)) {
            result = ASTRA_SYSCALL_INVALID_ARGUMENT;
            break;
        }
        handle_status = kernel_handle_lookup(
            current->handles, thread->context.data[1], KERNEL_OBJECT_PROCESS,
            required_right, (void **)&target);
        if (handle_status == KERNEL_HANDLE_INVALID_HANDLE ||
            handle_status == KERNEL_HANDLE_TYPE_MISMATCH) {
            result = ASTRA_SYSCALL_INVALID_HANDLE;
            break;
        ...
        if (syscall == ASTRA_SYSCALL_PROCESS_SIGNAL) {
            if (queue_process_signal(target, signal, NULL) !=
                KERNEL_PROCESS_OK)
                return KERNEL_PROCESS_CORRUPT;
        } else if (syscall == ASTRA_SYSCALL_PROCESS_TERMINATE) {
            bool self = target == current;
            KernelProcessStatus terminate_status = retire_process(
                target, KERNEL_PROCESS_EXIT_SIGNAL, signal, next_context);

            if (terminate_status != KERNEL_PROCESS_OK)
                return terminate_status;
            if (self)
                return terminate_status;
        } else if (syscall == ASTRA_SYSCALL_PROCESS_SUSPEND) {
            KernelThreadStatus thread_status;
            bool self = target == current;
    ...
    case ASTRA_SYSCALL_FUTEX_WAIT: {
        uint32_t address = thread->context.data[1];
        uint32_t expected = thread->context.data[2];
        uint32_t observed;
        uint64_t deadline_cycles;
        uint64_t now;
        KernelFutexStatus futex_status;
        ...
        if (kernel_vm_read(&current->address_space, address, &observed,
                           sizeof(observed)) != KERNEL_VM_OK) {
            result = ASTRA_SYSCALL_BAD_ADDRESS;
            break;
        }
        ...
        if (futex_status == KERNEL_FUTEX_INVALID_ARGUMENT) {
            result = ASTRA_SYSCALL_INVALID_ARGUMENT;
            break;
        }
        if (futex_status == KERNEL_FUTEX_NO_SLOT) {
            result = ASTRA_SYSCALL_RESOURCE_LIMIT;
            break;
        }
    ...
    case ASTRA_SYSCALL_FUTEX_WAKE: {
        uint32_t woken;
        KernelFutexStatus futex_status = kernel_futex_wake(
            current->id, thread->context.data[1], thread->context.data[2],
            ASTRA_SYSCALL_OK, &woken);
        ...
        if (futex_status != KERNEL_FUTEX_OK)
            return KERNEL_PROCESS_CORRUPT;
        thread->context.data[1] = woken;
        break;
    }
...
KernelProcessStatus kernel_process_maintenance(void)
{
    ...
    if (soak_state.enabled != 0u &&
        ...
        if (!soak_metadata_frames(&metadata_frames) ||
            !astra_u32_add_checked(memory_stats.free_frames,
                                   survivor_frame_delta,
                                   &accounted_frames) ||
            !astra_u32_add_checked(accounted_frames, metadata_frames,
                                   &accounted_frames) ||
            !astra_u32_add_checked(soak_state.baseline_free_frames,
                                   soak_state.baseline_metadata_frames,
                                   &expected_frames))
            return maintenance_failed(
                KERNEL_PROCESS_MAINTENANCE_OWNER_FRAME_UNDERFLOW,
                KERNEL_PROCESS_CORRUPT, memory_stats.free_frames,
                soak_state.baseline_free_frames);
        if (accounted_frames != expected_frames)
...
bool kernel_process_maintenance_pending(void)
{
    ...
    if (kernel_thread_reap_pending())
        return true;
    return exiting_process_count != 0u;
}
    ...
    uint32_t baseline_free_frames, uint32_t report_interval)
{
    ...
    soak_state.baseline_metadata_frames = metadata_frames;
