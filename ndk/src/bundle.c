...
static int utf8(const char *text, uint32_t length)
{
...
/* Shared lexer rules for Astra's line-oriented manifests. */
                                     ...
                                     uint32_t *error_line)
{
    ...
    if (text == NULL || manifest == NULL || length == 0u ||
        length > ASTRA_BUNDLE_MANIFEST_MAX || !utf8(text, length))
