/** @file event_emit.h @brief Typed, filtered structured-event emission. */
...
/** Lowest event level emitted by each subsystem. */
extern uint8_t astra_event_levels[ASTRA_EVENT_SUBSYSTEM_MAX];

/**
 * Set the minimum emitted level for one subsystem.
 * @param subsystem ASTRA_EVENT_SUBSYSTEM_* identifier.
 * @param level ASTRA_EVENT_LEVEL_* threshold.
 * @return ASTRA_SYSCALL_* status.
 */
uint32_t astra_event_level_set(uint32_t subsystem, uint32_t level);

/**
 * Test whether an event would pass the current subsystem threshold.
 * @param subsystem ASTRA_EVENT_SUBSYSTEM_* identifier.
 * @param level ASTRA_EVENT_LEVEL_* value.
 * @return Nonzero when the event is enabled; otherwise zero.
 */
static inline int
 ...
 */
/**
 * Emit an event with already separated word values.
 * @param descriptor Static event catalog descriptor.
 * @param level ASTRA_EVENT_LEVEL_* value.
...
/**
 * Emit a structured event with no occurrence-specific values.
 * @param subsystem ASTRA_EVENT_SUBSYSTEM_* identifier.
 * @param level ASTRA_EVENT_LEVEL_* value.
 * @param format Static event message format.
 */
...
/**
 * Emit a structured event with one occurrence-specific value.
 * @param subsystem ASTRA_EVENT_SUBSYSTEM_* identifier.
 * @param level ASTRA_EVENT_LEVEL_* value.
 * @param format Static event message format.
 * @param a0 First value, no wider than 32 bits.
 */
...
/**
 * Emit a structured event with two occurrence-specific values.
 * @param subsystem ASTRA_EVENT_SUBSYSTEM_* identifier.
 * @param level ASTRA_EVENT_LEVEL_* value.
 * @param format Static event message format.
 * @param a0 First value, no wider than 32 bits.
 * @param a1 Second value, no wider than 32 bits.
 */
...
/**
 * Emit a structured event with three occurrence-specific values.
 * @param subsystem ASTRA_EVENT_SUBSYSTEM_* identifier.
 * @param level ASTRA_EVENT_LEVEL_* value.
 * @param format Static event message format.
 * @param a0 First value, no wider than 32 bits.
 * @param a1 Second value, no wider than 32 bits.
 * @param a2 Third value, no wider than 32 bits.
 */
...
/**
 * Emit a structured event with four occurrence-specific values.
 * @param subsystem ASTRA_EVENT_SUBSYSTEM_* identifier.
 * @param level ASTRA_EVENT_LEVEL_* value.
 * @param format Static event message format.
 * @param a0 First value, no wider than 32 bits.
 * @param a1 Second value, no wider than 32 bits.
 * @param a2 Third value, no wider than 32 bits.
 * @param a3 Fourth value, no wider than 32 bits.
 */
