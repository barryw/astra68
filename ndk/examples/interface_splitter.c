    ...
    uint16_t width, uint16_t height)
{
    ...
    result = astra_interface_container_init(&controls[0], &pane);
    ...
    result = astra_interface_splitter_init(&controls[2], &splitter);
    if (result != ASTRA_OK) return result;
    item = (AstraFlexItem)ASTRA_FLEX_ITEM_INIT;
    ...
    pane.layout.padding_left = 8u;
    pane.layout.padding_top = 8u;
    result = astra_interface_container_init(&controls[3], &pane);
    if (result != ASTRA_OK) return result;
    item = (AstraFlexItem)ASTRA_FLEX_ITEM_INIT;
    item.minimum_width = 64u;
    item.grow = 2u;
