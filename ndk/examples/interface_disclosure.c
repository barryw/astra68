...
AstraResult astra_example_build_disclosure(
    const AstraInterfaceLibrary *interface, AstraControl controls[4],
    AstraUIContext *ui, uint16_t width, uint16_t height)
{
    ...
    if (controls == NULL || ui == NULL ||
        !astra_interface_library_supports(
            interface, 1u, ASTRA_INTERFACE_LIBRARY_5_1_SIZE))
    ...
    result = interface->checkbox_init(&controls[3], &option);
    if (result != ASTRA_OK) return result;
