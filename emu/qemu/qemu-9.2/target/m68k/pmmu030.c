...
static pmmu030_atc_entry *atc_lookup(pmmu030_state *state,
                                     ...
                                     uint8_t function_code)
{
    uint32_t page_mask = pmmu030_page_mask_inline(state);
    uint32_t logical_page = logical_address & ~page_mask;
    unsigned hint_slot =
        pmmu030_atc_hint_slot_inline(logical_page, function_code);
    pmmu030_atc_entry *hint_entry =
        pmmu030_atc_hint_inline(state, logical_page, function_code);
    unsigned i;
    ...
    for (i = 0u; i < PMMU030_ATC_ENTRIES; ++i) {
        ...
        if (entry->valid && entry->function_code == (function_code & 7u) &&
            ...
            state->atc_hints[hint_slot] = (uint8_t)(i + 1u);
            return entry;
        }
    }
    state->atc_hints[hint_slot] = 0u;
    return NULL;
}
                                       ...
                                       uint8_t function_code)
{
    ...
    entry->logical_page = logical_address & ~pmmu030_page_mask_inline(state);
    {
        unsigned hint_slot = pmmu030_atc_hint_slot_inline(
            entry->logical_page, entry->function_code);
        state->atc_hints[hint_slot] =
            (uint8_t)((unsigned)(entry - state->atc) + 1u);
    }
    return entry;
}
...
{
    return pmmu030_translation_active_inline(state);
}

unsigned pmmu030_page_shift(const pmmu030_state *state)
{
    return pmmu030_page_shift_inline(state);
}

uint32_t pmmu030_page_mask(const pmmu030_state *state)
{
    return pmmu030_page_mask_inline(state);
}
...
void pmmu030_flush_all(pmmu030_state *state)
{
    ...
    memset(state->atc_hints, 0, sizeof(state->atc_hints));
