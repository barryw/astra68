...
(
    ...
    output logic [3:0][55:0] sub
);

// Connect the current packet type's data to the output.
logic [7:0] packet_type = 8'd0;
logic [23:0] headers [255:0];
logic [255:0][3:0][55:0] subs;
...
audio_clock_regeneration_packet #(.VIDEO_RATE_HZ(VIDEO_RATE_HZ), .AUDIO_RATE(AUDIO_RATE)) audio_clock_regeneration_packet (.clk_pixel(clk_pixel), .clk_audio(clk_audio), .clk_audio_counter_wrap(clk_audio_counter_wrap), .header(headers[1]), .sub(subs[1]));
...
logic [1:0][AUDIO_BIT_WIDTH-1:0] audio_sample_word_transfer;
...
logic [1:0][3:0][1:0][23:0] audio_sample_word_buffer;
logic [1:0][AUDIO_BIT_WIDTH-1:0] audio_sample_word_transfer_mux;
always_comb
begin
    if (audio_sample_word_transfer_control_synchronizer_chain[0] ^ audio_sample_word_transfer_control_synchronizer_chain[1])
        audio_sample_word_transfer_mux = audio_sample_word_transfer;
    else begin
        audio_sample_word_transfer_mux[0] = audio_sample_word_buffer[sample_buffer_current][samples_remaining][0][23:(24-AUDIO_BIT_WIDTH)];
        audio_sample_word_transfer_mux[1] = audio_sample_word_buffer[sample_buffer_current][samples_remaining][1][23:(24-AUDIO_BIT_WIDTH)];
    end
end
...
logic [3:0][1:0][23:0] audio_sample_word_packet;
