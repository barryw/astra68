...
if {[info exists ::env(ASTRA_OOC_OUT)]} {
    ...
    set output_dir [file normalize [file join $graphics_dir ../../../build/arty-graphics/render-geometry-ooc]]
}

file mkdir $output_dir
read_verilog -sv [file join $graphics_dir astra_render_geometry.sv]
read_xdc [file join $script_dir render_command_ooc.xdc]
synth_design -top astra_render_geometry -part xc7z020clg400-1 \
...
write_checkpoint -force \
    [file join $output_dir astra_render_geometry_routed.dcp]

set path [get_timing_paths -quiet -group render_clk \
    -delay_type max -max_paths 1 -nworst 1]
if {[llength $path] == 0} {
    puts "ASTRA_RENDER_GEOMETRY_OOC ERROR no render-clock timing path"
    exit 3
}
set slack [get_property SLACK $path]
set startpoint [get_property STARTPOINT_PIN $path]
set endpoint [get_property ENDPOINT_PIN $path]
puts "ASTRA_RENDER_GEOMETRY_OOC period_ns=5.000 setup_slack_ns=$slack start=$startpoint end=$endpoint"
if {$slack < 0.0} {
    exit 2
}
