...
if {[info exists ::env(ASTRA_OOC_OUT)]} {
    ...
    set output_dir [file normalize [file join $graphics_dir ../../../build/arty-graphics/sprite-line-ooc]]
}

file mkdir $output_dir
read_verilog -sv [file join $graphics_dir astra_premult_blend.sv]
read_verilog -sv [file join $graphics_dir astra_sprite_line_store.sv]
read_verilog -sv [file join $graphics_dir astra_sprite_line_builder.sv]
read_xdc [file join $script_dir sprite_line_ooc.xdc]
synth_design -top astra_sprite_line_builder -part xc7z020clg400-1 \
...
if {[llength $bram_cells] == 0} {
    puts "ASTRA_SPRITE_LINE_OOC ERROR no block RAM primitives inferred"
    exit 4
}
...
write_checkpoint -force \
    [file join $output_dir astra_sprite_line_builder_routed.dcp]
...
if {[llength $build_path] == 0} {
    puts "ASTRA_SPRITE_LINE_OOC ERROR no build-clock timing path"
    exit 3
}
...
puts "ASTRA_SPRITE_LINE_OOC bram_primitives=[llength $bram_cells] build_period_ns=5.000 build_setup_slack_ns=$build_slack"
puts "ASTRA_SPRITE_LINE_OOC pixel_period_ns=13.468 pixel_setup_slack_ns=$pixel_slack"
if {$build_slack < 0.0 ||
