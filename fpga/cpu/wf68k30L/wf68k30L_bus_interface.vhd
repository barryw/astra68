...
begin
    ...
    P_SYNC: process(CLK)
    -- These flip flops synchronize the bus termination signals. Originally sampled on the
    -- NEGATIVE clock edge ("end of S2") for the dual-edge half-clock timing; now sampled on
    -- the POSITIVE edge together with the rest of the (single-edge) bus interface. With the
    -- full-clock S0..S5 states these signals are still valid well before they are used at S3.
    -- Be aware, that we have to buffer the RETRY signal to prevent the bus controller of
    -- spurious or timing critical BERRn and/or HALTn signals.
    ...
    IN_MUX: process
    -- This is the input multiplexer which can handle up to four bytes. Originally latched on
    -- the NEGATIVE edge (mid-S4); now on the POSITIVE edge (end of S4) with the single-edge
    -- full-clock states. The slave holds read data stable across the cycle, so posedge
    -- capture at S4 is in time for PREFETCH_BUFFERS at S5.
    begin
    wait until CLK = '1' and CLK' event;
        --
    ...
    SLICES: process
    -- SINGLE-EDGE rewrite of the central bus timing (was dual-edge).
    -- Original: SLICE_CNT_P advanced on the positive edge and SLICE_CNT_N followed it on the
    -- NEGATIVE edge, so each S0..S5 state was HALF a clock (async bus cycle = 3 clocks). That
    -- negedge follower plus the combinational T_SLICE decode is a cross-edge hazard that does
    -- not close timing on ECP5 (Diamond scores it 0-hold, but it races on real silicon and
    -- the design never boots). Here T_SLICE is a single positive-edge state register: each
    -- state is a FULL clock (async bus cycle = 6 clocks). Removing the negedge/cross-edge
    -- paths roughly doubles the achievable clock, so net throughput is about the same, and
    -- the whole core is now single-edge -> a normal synchronous design the tools hold-fix.
    -- BUS_CYC_RDY (unchanged) still asserts at S3 (synchronous) / S5 (asynchronous).
    begin
        wait until CLK = '1' and CLK' event;
        if RESET_CPU_I = '1' then
            T_SLICE <= IDLE;
            ...
            T_SLICE <= IDLE; -- Cycle terminating: async done at S5 or sync done at S3.
        else
            case T_SLICE is
                when IDLE   => T_SLICE <= S0; -- BUS_CTRL_STATE /= IDLE: a bus cycle is active.
                when S0     => T_SLICE <= S1;
                when S1     => T_SLICE <= S2;
                when S2     => T_SLICE <= S3;
                when S3 =>
                    if STERM_In = '0' then
                        T_SLICE <= S0;  -- Synchronous cycle: next portion (completion handled above).
                    elsif WAITSTATES = '1' then
                        T_SLICE <= S3;  -- Asynchronous cycle: wait here until DSACK is sampled.
                    else
                        T_SLICE <= S4;
                    end if;
                when S4     => T_SLICE <= S5;
                when S5     => T_SLICE <= S0; -- Next portion of a multi-transfer (completion handled above).
                when others => T_SLICE <= IDLE;
            end case;
        end if;
    end process SLICES;
