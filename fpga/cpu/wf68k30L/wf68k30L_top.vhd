...
entity WF68K30L_TOP is
    ...
    port (
        ...
        -- DEBUG taps (HW bring-up): expose the internal read/immediate/AR-write datapath.
        DBG_D2C         : out std_logic_vector(31 downto 0); -- DATA_TO_CORE (assembled read data)
        DBG_IMM         : out std_logic_vector(31 downto 0); -- DATA_IMMEDIATE (immediate operand)
        DBG_ARIN        : out std_logic_vector(31 downto 0)  -- AR_IN_1 (address-register write value)
    );
...
begin
    ...
    -- DEBUG taps (HW bring-up): opcode-delivery + decode (why hang after prefetch).
    --   D2C  = [15:0]OPCODE_TO_CORE(word bus->pipe); nib4 {EW_REQ,OW_REQ,OPCODE_RDY}=4/2/1(bit16=RDY);
    --          [16]OPCODE_RDY [17]OW_REQ [18]EW_REQ [19]EW_ACK [20]IPIPE_FILL [21]IPIPE_FLUSH [22]OPD_ACK
    --   IMM  = [31:16]BIW_1  [15:0]BIW_0   (latched instruction words)
    --   ARIN = [31:16]EXT_WORD [15:0]BIW_2
    DBG_D2C(15 downto 0) <= OPCODE_TO_CORE;
    DBG_D2C(16) <= '1' when OPCODE_RDY   = '1' else '0';
    DBG_D2C(17) <= '1' when OW_REQ_MAIN  = '1' else '0';
    DBG_D2C(18) <= '1' when EW_REQ_MAIN  = '1' else '0';
    DBG_D2C(19) <= '1' when EW_ACK       = '1' else '0';
    DBG_D2C(20) <= '1' when IPIPE_FILL   = '1' else '0';
    DBG_D2C(21) <= '1' when IPIPE_FLUSH  = '1' else '0';
    DBG_D2C(22) <= '1' when OPD_ACK_MAIN = '1' else '0';
    DBG_D2C(31 downto 23) <= (others => '0');
    DBG_IMM  <= BIW_1 & BIW_0;
    DBG_ARIN <= EXT_WORD & BIW_2;
    ...
    -- Refill/status outputs. Was negative-edge; now positive-edge so the whole core is
    -- single-edge (these are output-only status signals, unused in the Astra 68 SoC).
    begin
        wait until CLK = '1' and CLK' event;
