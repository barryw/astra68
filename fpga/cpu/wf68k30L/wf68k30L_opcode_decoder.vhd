...
signal PC_INC_I             : bit;
signal PIPE_RDY             : bit;
-- Were process variables in P_PC_OFFSET; promoted to signals so the combinational PC_OFFSET/
-- PC_ADR_OFFSET readout lives in its own process (removes the mixed clocked/comb process).
-- Signal semantics also inherently give ADR_OFFSET the 'old' PC_VAR the original relied on ordering for.
signal ADR_OFFSET           : std_logic_vector(6 downto 0);
signal PC_VAR               : std_logic_vector(6 downto 0);
signal PC_VAR_MEM           : std_logic_vector(6 downto 0);
begin
    ...
    -- Clocked part: ADR_OFFSET/PC_VAR/PC_VAR_MEM are now signals (were variables), split out of
    -- the old mixed clocked+combinational process so every tool synthesizes it cleanly. Signal
    -- semantics inherently give ADR_OFFSET the 'old' pre-update PC_VAR the original relied on
    -- statement ordering for.
    P_PC_OFFSET: process
    begin
        wait until CLK = '1' and CLK' event;
            if IPIPE_FLUSH = '1' then
    ...
    -- Combinational part: PC_OFFSET / PC_ADR_OFFSET are pure functions of the registered
    -- PC_VAR_MEM / PC_VAR / ADR_OFFSET (was the tail of the old mixed process).
    P_PC_OFFSET_COMB: process(BUSY_EXH, PC_INC_I, OP, LOOP_BSY_I, LOOP_EXIT, PC_VAR_MEM, PC_VAR, ADR_OFFSET)
    begin
