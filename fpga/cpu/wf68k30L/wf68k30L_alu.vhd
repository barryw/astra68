...
begin
    ...
    variable DVD_SIGN       : std_logic; -- dividend sign  (remainder takes this sign, DIVS)
    variable DVS_SIGN       : std_logic; -- divisor  sign  (quotient sign = dividend xor divisor, DIVS)
    ...
    begin
        ...
        case DIV_STATE is 
            ...
            when INIT =>
                ...
                -- Latch operand signs for result sign correction (DIVS). The remainder
                -- takes the sign of the dividend; the quotient the xor of both signs.
                if OP_SIZE = LONG and BIW_1(10) = '1' then
                    DVD_SIGN := OP3(31); -- 64 bit dividend
                else
                    DVD_SIGN := OP2(31); -- 32 bit dividend (also DIVS.W: full 32 bit in OP2)
                end if;
                ...
                if DIVISOR = x"00000000" then -- Division by zero.
                    ...
                    DIV_STATE <= IDLE;
                    DIV_RDY <= '1';
                elsif x"00000000" & DIVISOR > DIVIDEND then -- Divisor > dividend: quotient 0, remainder = dividend.
                    if OP = DIVS and DVD_SIGN = '1' then
                        REMAINDER <= not DIVIDEND(31 downto 0) + 1; -- negative dividend => negative remainder
                    else
                        REMAINDER <= DIVIDEND(31 downto 0);
                    end if;
                    DIV_STATE <= IDLE;
                    DIV_RDY <= '1';
                elsif x"00000000" & DIVISOR = DIVIDEND then -- |dividend| = |divisor|: quotient +/-1, remainder 0.
                    if OP = DIVS and (DVD_SIGN xor DVS_SIGN) = '1' then
                        QUOTIENT <= x"FFFFFFFF"; -- -1: signs differ
                    else
                        QUOTIENT <= x"00000001";
                    end if;
                    DIV_STATE <= IDLE;
