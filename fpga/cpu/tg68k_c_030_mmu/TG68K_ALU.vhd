...
--	signal i						: std_logic_vector(5 downto 0);
	...
	function divs_overflow_flags_68020(
		...
		quotient_low : std_logic_vector(7 downto 0)
	) return std_logic_vector is
		variable flags        : std_logic_vector(3 downto 0) := (others => '0');
	...
	begin
		...
		-- The iterative divider has already computed the absolute quotient. Reuse
		-- its low byte here; a VHDL "/" creates a full combinational divider and
		-- puts this flag calculation on the ALU's single-cycle critical path.
		if quotient_low = x"00" then
			flags(2) := '1';
		end if;
		if quotient_low(7) = '1' then
			flags(3) := '1';
		end if;
...
process (OP1out, OP2out, opcode, bit_nr, bit_msb, bs_shift, bs_shift_mod, ring, result_bs, exe_opcode, vector, 
	...
	variable asr_sign_v : std_logic_vector(32 downto 0);
	begin
		bs_C <= '0';
		msb <= '0';
		ring <= "100000";
...
-- calc ASR sign
		...
		FOR i IN 1 TO 32 LOOP
			asr_sign_v(i) := asr_sign_v(i - 1) OR hot_msb(i - 1);
		END LOOP;
		asr_sign <= asr_sign_v;
						...
						IF V_Flag='1' THEN	
							IF CPU(1)='1' THEN
								IF div_word_latched='1' THEN
									...
									ELSE
										Flags(3 downto 0) <= divs_overflow_flags_68020(
											div_dividend_latched(47 downto 16),
											div_src_latched(15 downto 0),
											div_quot(7 downto 0)
										);
									END IF;
								ELSIF div_signed_latched='0' THEN
...
----lower bound first
						...
						ELSIF signed(chk2_val) > signed(chk2_upper) AND signed(chk2_val) < signed(chk2_lower) THEN
							Flags(0) <= '1';
						END IF;
						-- Astra's directed tests and the no-MMU TG candidate both rely on
						-- deterministic zeroes for the architecturally unhelpful N/V bits.
						Flags(1) <= '0';
						Flags(3) <= '0';
