...
--
-- Per MC68030 User's Manual Table 8-6:
...
--                          cpTRAPcc, MMU Configuration

library ieee;
use ieee.std_logic_1164.all;
use ieee.numeric_std.all;

entity tb_stack_frame_push_motorola is
end entity;

architecture behavior of tb_stack_frame_push_motorola is
    ...
    signal debug_fline_opcode_pc_sig : std_logic_vector(31 downto 0);
...
begin
    ...
    dut: entity work.TG68KdotC_Kernel
        ...
        port map(
            ...
            debug_fline_opcode_pc => debug_fline_opcode_pc_sig
        );
    ...
    test: process
        ...
        procedure check_instruction_address(
            test_name : string;
            expected_address : std_logic_vector(31 downto 0)
        ) is
            variable actual_address : std_logic_vector(31 downto 0);
        begin
            actual_address := v_instr_addr_hi & v_instr_addr_lo;
            if actual_address = expected_address then
                report "  PASS: " & test_name & " instruction_address=$" &
                       integer'image(to_integer(unsigned(expected_address))) severity note;
                test_passed <= test_passed + 1;
            else
                report "  FAIL: " & test_name & " instruction address: expected=$" &
                       integer'image(to_integer(unsigned(expected_address))) &
                       " got=$" & integer'image(to_integer(unsigned(actual_address))) severity error;
    ...
    begin
        ...
        -- ================================================================
        -- TEST 11: MMU Configuration Exception - Format $2, Vector 56 ($E0)
        -- Table 8-6 requires the PMOVE instruction address in the six-word frame.
        -- ================================================================
        report "" severity note;
        report "TEST 11: MMU Configuration (Format $2, Vector $E0)" severity note;
        ...
        v_sp_final := x"00003FF4";
        read_frame(v_sp_final, "0010");
        report "  DIAG: latched F-line opcode PC=$" &
               integer'image(to_integer(unsigned(debug_fline_opcode_pc_sig))) severity note;
        check_format("MMU Configuration", "0010");
        check_vector("MMU Configuration", x"0E0");
        check_pc("MMU Configuration PC=next_instruction", x"0000100A");
        check_instruction_address("MMU Configuration", x"00001006");
